...
**State:** Gate 1 CLOSED (no Tier-1 identifier found); Gate 2 redesigned and scheduled, not yet run; a third, narrower diagnostic separately authorized and scheduled, not yet run — **Owner:** SE (analytical/design), SDT-HF (implementation) — **updated 2026-09-04, verified directly against live scheduled-task state and `histfints` commit history, not taken on report.**

**Gate 1 finding, `histfints@d92145d`'s own record.** Direct inspection of a live BYMA cedears panel response found **no date/business-date/session-status field of any kind** — Tier-1 (an explicit session-date change) evidence is unavailable from this endpoint. Rather than closing the investigation outright per this item's original binary framing, SE/SDT-HF introduced a graduated evidence framework: Tier-2 (cross-symbol multi-field reset) and Tier-3 (next-session trade confirmation) may still be evaluable **after the fact** from a complete raw record, even without an explicit Tier-1 field. **This refinement happened directly between SE and SDT-HF; RGC reconstructs it here from the committed record, not from having relayed it — flag any imprecision for correction.**

**Gate 2, redesigned on that basis, `histfints@d92145d`.** The original task was reconfigured (not newly created) to `run_byma_publication_window_measurement_v2.bat`, capturing the **complete raw row** per tracked symbol per poll (not a curated `tradeHour`/price subset) plus the HTTP response's own `Date` header, so Tier-2/Tier-3 evidence can be extracted post-hoc. **Verified live via Task Scheduler just now**: enabled, bounded `2026-09-07T10:00` → `2026-09-09T10:00` ART, 15-minute repetition, `StopAtDurationEnd=True` — scheduled, has not yet run (`LastRunTime` still the never-run placeholder).
...
7. **Gate closure criteria.** **Gate 1 — CLOSED**: no Tier-1 identifier found (above). **Gate 2 — scheduled, not yet closed**: redesigned to capture full raw rows for post-hoc Tier-2/Tier-3 evaluation; closes when the 2026-09-07–09-09 run completes and SE evaluates the result.
8. **Open questions, named GAP owner.** GAP — SE: once Gate 2's run completes, do the captured raw rows actually support a Tier-2 or Tier-3 conclusion, or does the investigation end at "unanswerable from this panel" after all?

**Third, narrower diagnostic — separately authorized, not blocked by Gate 2, `histfints@98f1f50` (SE-approved).** Measures observable `closingPrice` publication/persistence/reversion (`0.0 → nonzero → persistence → 0.0`) for the five tickers PO reviewed against the 63-poll baseline (MU/MSFT/AMD/MELI/QQQ, LOCAL/ARS settlement only) — bounded 24 hours, 15-minute continuous polling, **verified live via Task Scheduler**: enabled, `2026-09-04T23:00` → `2026-09-05T22:56` ART, `StopAtDurationEnd=True`, not yet run. **Permitted conclusion**: observable `closingPrice` publication/persistence/reversion bounds, reported as a sampling interval (last-known-old, first-known-new) per ticker, never a single forced timestamp. **Prohibited conclusion from this test alone**: an official BYMA session rollover, business-date transition, or identification of which session a carried value belongs to. **No qualifying session/date identifier is required for this narrower measurement** — it makes no session-identification claim at all, which is why it isn't blocked by Gate 2 still being open.

**Self-expiry, not a dependency of this entry.** The original task's own `EndBoundary` is 2026-09-22 — it stops firing on its own regardless of progress here. This entry stays open until Gate 2 and the third diagnostic both resolve.
