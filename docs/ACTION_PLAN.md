...
**State:** Gate 1 CLOSED (no Tier-1 identifier found); Gate 2 redesigned but currently DISABLED (scheduling-collision avoidance, not abandonment); a third, narrower diagnostic rescheduled to an ordinary trading-day pair, not yet run — **Owner:** SE (analytical/design), SDT-HF (implementation) — **updated 2026-09-04, verified directly against live scheduled-task state and `histfints` commit history, not taken on report.**
...
**Gate 2, redesigned on that basis, `histfints@d92145d`.** The original task was reconfigured (not newly created) to `run_byma_publication_window_measurement_v2.bat`, capturing the **complete raw row** per tracked symbol per poll (not a curated `tradeHour`/price subset) plus the HTTP response's own `Date` header, so Tier-2/Tier-3 evidence can be extracted post-hoc. Bounded `2026-09-07T10:00` → `2026-09-09T10:00` ART, 15-minute repetition, `StopAtDurationEnd=True`.

**Gate 2 now DISABLED, not deleted or closed, `verified live via Task Scheduler`.** Disabled to prevent duplicate polling against the third diagnostic below, which now occupies the same Monday 10:00 start — **not** because Gate 2's design is abandoned. Its stronger session-identifier approach remains unsupported by Gate 1's finding; disabling only removes a scheduling collision, and re-enabling is SE's/SDT-HF's call whenever Gate 2 is actually run.
