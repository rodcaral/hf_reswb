...
### D-047 — Windows GUI: frozen legacy surface, no parity requirement, reactivation gated on demonstrated Web-architecture inadequacy

*Decided 2026-09-02. PO scope decision, persisted per SE relay. Requires no further DFA/PO
approval unless a future reactivation proposal satisfies the criterion stated below.*

**Status: frozen legacy surface.** HistFinTS's Windows GUI (`presentation/gui.py`) receives **no
new feature development** and carries **no parity requirement** with the Web UI. It is preserved
in repository history/reference for now — not deleted, not refactored, not actively maintained
toward feature currency.

**Reactivation criterion — the only basis for reconsidering active GUI development:**

> Reconsider active Windows GUI development only if a concrete required capability is demonstrated
> to be materially inadequate in the Web architecture.

Examples that **may** justify reconsideration (a demonstration, not a presumption, is still
required in each case):
- demonstrated browser-performance limits at the required research/chart workload;
- a genuine offline/local-state requirement;
- essential OS/device integration;
- another concrete deployment/security constraint the Web UI cannot reasonably satisfy.

**Explicitly insufficient by themselves — stated so these cannot be mistaken for satisfying the
criterion above:**
- the existence of the current desktop GUI;
- anticipated chart/indicator complexity, absent a demonstrated Web limitation;
- a preference for native UI;
- a desire for feature parity with the GUI.

**Retirement safeguard, binding before eventual deletion:** before the GUI is ever deleted,
identify any reusable non-UI financial/domain logic or chart-calculation logic currently residing
inside it and, where valuable, extract that logic into shared/core components rather than
retaining the GUI merely as a container for it. This safeguard applies whenever deletion is
eventually proposed — it is not itself an instruction to begin that extraction now.

**Explicitly not authorized by this decision**: no GUI deletion or refactor now; no parity backlog
created; INC-17's own execution is unaffected and unchanged by this decision.
...
### DOM-1 — Dispersion threshold: UNCALIBRATED. `CV 0.167` superseded as current normative
language in `SPEC_PANEL_ELIGIBILITY.md`

*Issued 2026-09-04. DFA's own ruling, persisted per SE/PO relay — DOM-1 is DFA's own governance
identifier, recorded here verbatim, not renumbered into this log's own `D-###` sequence (the
same convention already applied to `F-###` defect identifiers throughout this log).*

**Governing current state, recorded exactly as instructed**:
...
**Supersedes, explicitly, every current normative statement previously asserting**:
`dispersion_threshold = 0.167` as a current operational parameter; that `0.167` may proceed
analytically; that `0.167` is sufficient for PRIMARY-cohort analytical use; that dispersion
presently has an operational numerical threshold; that raw-price coefficient of variation
remains the current dispersion methodology.

**Specification corrected in the same change**: `docs/SPEC_PANEL_ELIGIBILITY.md` — a new
2026-09-04 update block added at the top of the document (the established convention that
document already uses for dated corrections), and every normative passage in §8.3/§8.5/the
document's second "§8. Open items" section asserting any of the above is marked
...
**Repository-wide, read-only impact trace performed** — full detail:
`docs/evidence/DOM1_DISPERSION_THRESHOLD_IMPACT_TRACE_2026-09-04.md`. Summary: **zero
occurrences of `0.167` (or a materially equivalent hard-coded/default representation) found
anywhere in `src/` or `tests/`** — `dispersion_threshold` is a caller-supplied
`Optional[DispersionThreshold] = None` field throughout `hf_reswb`'s domain/application code,
with no default value and no call site anywhere in this codebase constructing one with `0.167`.
**Category 3 (unauthorized current default/runtime parameter): none found. Category 4 (actual
decision-bearing consumption): none found.** Every one of the fifteen files containing the
literal string `0.167` is a documentation file (`ACTION_PLAN.md`, `DECISIONS.md`,
`SPEC_PANEL_ELIGIBILITY.md`, and eleven `docs/evidence/*.md` historical calibration-attempt
records) — Category 1 (historical/non-decision-bearing), each already dated and self-qualified
as provisional/not-promoted at the time it was written, none cited anywhere as current
authoritative guidance.

**Confirmed explicitly, verified rather than assumed**: **no accepted bounded INC-7 result
consumed `0.167`.** INC-7's own accepted implied-FX diagnostics (`§15b/§15f/§15g/§15h/§15i`)
execute entirely inside HistFinTS's own `implied_fx.py`/`ratio_applicability_service.py` module
chain — structurally separate from, and never calling into, `hf_reswb`'s panel/dispersion
modules, confirmed by this session's own repeated module-boundary review throughout the INC-7
Gate A conformance work. Within `hf_reswb` itself, `dispersion_threshold` is never set to
`0.167` (or any value) by any code path — confirmed by direct source search, not inferred.

**Preserved unaffected by this ruling**: §15d's own RETAIN AS UNVERIFIED / NON-DECISION-BEARING
HISTORICAL ARTIFACT downgrade of the 2026-08-18 dispersion/CV calibration — DOM-1 reaffirms it,
does not reopen or relitigate it. Every accepted bounded INC-7 result (§15b/§15f/§15g/§15h/§15i)
remains exactly as recorded, none reopened. Cross-sectional dispersion methodology design
(§15j/§15k, `LOG_RELATIVE` primary, provisional median design-only) remains exactly as recorded,
unaffected — DOM-1 confirms, not changes, that design's own already-stated calibration-deferred
status.
...
### DOM-3 — Historical cadence evidence sufficiency: four requirements for qualifying evidence

*Settled, relayed by SE 2026-09-04. First appearance of this identifier in this repository —
confirmed by direct search of `DECISIONS.md` and `ACTION_PLAN.md` before recording; recorded
here exactly as relayed and attributed to DFA via SE, not self-certified or independently
derived by SDT-WB.* Persisted here per this project's own established convention of recording
every DOM-numbered ruling verbatim (DOM-1, DOM-2's own precedent), not left to stand only in a
relayed instruction.

**Four requirements, recorded exactly as instructed, that a candidate piece of evidence must
satisfy to qualify as sufficient for establishing a Series' historical observation cadence for
a specific range** (the open question DOM-2 itself left unresolved — §1 DOM-2, `ACTION_PLAN.md`
§15r):

1. **Cadence assertion** — the evidence must actually assert a cadence/interval fact, not
   merely be consistent with one.
2. **Temporal applicability** — the assertion must apply to the specific historical range in
   question, not be projected backward or forward from an unrelated period.
3. **Traceable Series/provider linkage** — the evidence must connect to the specific Series
   (and, where relevant, the specific provider/acquisition mechanism) under examination, not be
   a general statement with no established connection to it.
4. **Compatibility with stored observations without unresolved contradiction** — the evidence
   must not conflict with what is actually stored for that Series/range in a way that is left
   unresolved.

**Governs evidence sufficiency only — does not itself resolve any specific Series' cadence.**
Applying these four requirements to Series 11312's `2000-12-20`–`2001-01-02` range is recorded
separately at §15t (`ACTION_PLAN.md`) / this changelog's own entry for that discovery task.
**Explicitly does not authorize inferring historical cadence merely because stored observations
appear daily** — the same prohibition DOM-2 already stated; DOM-3 supplies the sufficiency test,
...
| ~~Q-061~~ | closed | → D-042 (`include_delisted`/`staleness_policy`/`dispersion_threshold` specified, `SPEC-panel-eligibility.md` §8.1–§8.3), D-046 (implementation authorized). **Corrected 2026-09-04, per DFA's review** — this row still read "open · gates panel-eligibility implementation" after both had already landed. |
...
| Date | Change |
|---|---|
| 2026-09-04 | **SE correction to the DOM-3 cadence-evidence discovery for Series 11312 — `ACTION_PLAN.md` §15u, §5, §20.** Per SE's review of commit `c9b58b3`. The prior entry (below, unedited) and `ACTION_PLAN.md` §15t are preserved completely as written — this is an additive correction, not a rewrite. **DOM-3 itself confirmed accurately recorded by SE, unaffected — the correction is to §15t's own application of it, not to DOM-3.** **Two independent defects identified by SE, both confirmed correct on review**: (1) Yahoo's documented inability to return ~26 years of `1h` history is negative evidence against `1h`, not positive evidence that `1d` was actually requested — eliminating one candidate does not establish which of the remaining candidates (daily, weekly, monthly, other non-`1h` intraday) was actually used; DOM-3 requires a positive cadence assertion, which this does not supply. (2) `observation.import_run_id` is mutable last-writer provenance (already established at §15r/`F033_RETEST_2026-09-04.md` §7), reassigned by later upserts — it cannot establish that `import_run.id=25552` is the run that *originally acquired* the affected observations, only that it currently references them; this defeats the temporal-applicability and traceable-linkage steps §15t built on that citation. **Amended disposition, recorded exactly as instructed**: the Yahoo `~730`-day `1h` lookback argument is downgraded from `QUALIFYING` to **`CORROBORATING_ONLY`**; the claimed conclusion that `1d` is established for `2000-01-03`–`2026-08-14` is **withdrawn**; Series 11312, including `2000-12-20`–`2001-01-02`, **remains `UNRESOLVED` under DOM-3** pending positive cadence evidence; **no Workbench F-034 implementation proceeds from this evidence** — no cadence-assertion table or other representation is created, since the representation question is downstream of an admissible evidence source and none is established; creating one now would manufacture evidence HistFinTS does not possess. Both previously-proposed technical representations (§15t (a)/(b)) remain unimplemented and are now additionally moot for this range pending a genuine qualifying candidate — not merely deferred pending a design choice. Historical-status correction (item 3) remains held until `F-034` is resolved, unaffected by this correction; `F-033` quarantine integration (§15s) unaffected — this correction touches only §15t's own DOM-3 application. **No new candidate evidence sought or found in this correction** — a classification correction of evidence already gathered, not a fresh discovery pass. **No code changed. No HistFinTS or `histfints_uiue` file modified.** |
| 2026-09-04 | **Recorded DOM-3 — historical cadence evidence sufficiency, four requirements — and performed the DOM-3 cadence-evidence discovery for Series 11312's `2000-12-20`–`2001-01-02` range — §1 DOM-3, `ACTION_PLAN.md` §15t, §5, §20.** Per SE's relay, per DFA. §15/§15a–§15s preserved completely unedited. **DOM-3 is this repository's first recorded appearance of the identifier** — confirmed by direct search of both `DECISIONS.md` and `ACTION_PLAN.md` before recording, persisted per the same convention already applied to DOM-1/DOM-2. **DOM-3's four requirements, recorded exactly as instructed**: cadence assertion; temporal applicability; traceable Series/provider linkage; compatibility with stored observations without unresolved contradiction. **Governs evidence sufficiency only, does not itself resolve any Series' cadence.** **Discovery task performed, read-only, no F-034/historical-status implementation**: every candidate found for Series 11312's `2000-12-20`–`2001-01-02` range classified against the four requirements. `entity_change_log` (HistFinTS's own audit table) — `NOT_APPLICABLE`, zero rows for `entity_type='Series', entity_id=11312`, and the `configured_interval` field has never been logged for any Series repository-wide (0 rows). `series.configured_interval`/`import_run` schema — `NOT_APPLICABLE`, confirm rather than resolve DOM-2's own already-established gap (no per-date history, no interval column on `import_run` at all). `provider_assignment`/`provider` — `CORROBORATING_ONLY` (establishes series 11312 → Yahoo Finance, assignment `11314`, priority 1; asserts no cadence itself). The Class D "seven pairs" `1h`/`1d` sampling-configuration ruling — `NOT_APPLICABLE` (a different `2026-05-28` regime boundary for an unnamed cohort; series 11312 confirmed absent from that document by direct search). Bare observation timestamp/row-shape (one bar/day, fixed `14:00 UTC`) — `NOT_APPLICABLE` per instruction, explicitly not treated as sufficient regardless of uniformity. **One `QUALIFYING` candidate found**: `histfints/docs/PROVIDERS_AND_ADAPTERS.md`'s documented real `yahoo_finance` `1h`/`60m` lookback constraint (`~730` days; beyond it, the entire import fails outright with HTTP 422, explicitly no silent truncation; only daily/weekly/monthly carry no such limit), combined with the actual, directly-queried `import_run.id=25552` that produced every one of series 11312's `2000-01-03`–`2026-08-14` observations (6,624 rows, `provider_assignment_id=11314`, `trigger_type=MANUAL`, `status=PARTIAL`, `started_at=2026-08-15T16:26:01Z`) — the requested range sits entirely inside this run's confirmed output (rows `27827671`–`27827684` verified present). **Applying the four requirements, recorded exactly**: cadence assertion — the adapter documentation asserts a real technical capability fact, independent of the stored rows' own shape; temporal applicability — a `1h` request reaching `2000-01-03` from this run's actual `2026-08-15` execution date is `~13×` the documented limit and would have failed the entire run, yet the run's status is `PARTIAL` (not `FAILED`) with 6,624 real rows written, only possible under an interval the same documentation describes as carrying no such limit; traceable linkage — series 11312 → `provider_assignment 11314` → `yahoo_finance`, confirmed directly; compatible without contradiction — the subsequent `SCHEDULED` runs beginning `2026-08-14T15:00:00` (six-to-seven rows/day) mark the independently-observable point the interval changed, consistent with, not contradicting, this finding. **Applicable interval and outer bound recorded exactly as supported, not extended past it**: daily (`1d`), for at least `2000-01-03` through `2026-08-14T14:00:00` — run `25552`'s own actual output boundary. **Confidence caveat recorded explicitly, not smoothed over**: the `~730`-day figure is asserted in HistFinTS's own committed documentation, corroborated by an independent operational reference elsewhere in that repository (an `~8`-day `1m` limit relied upon for an unrelated suspension-safety argument), but no first-party "verified live" annotation or dated test log for the specific `730`-day figure was found, unlike several sibling rows in the same documentation table — this finding is `QUALIFYING` on DOM-3's own structural requirements, not independently re-verified by SDT-WB against the live Yahoo endpoint. **No candidate found `CONTRADICTORY`.** **Smallest viable technical representation proposed, explicitly NOT implemented**: (a) a new, additive, Workbench-owned local cadence-assertion table (never a HistFinTS write), consulted by `classify_series` for the requested range before falling through to the current series-global guard, populated only by this kind of explicit reviewed discovery, never inferred automatically from row shape — stays entirely inside Workbench's own write boundary (D-001); (b) a HistFinTS-side per-range `configured_interval` history mechanism (§15r's option (c)/(d)) would be more architecturally correct but is out of Workbench's own write authority — would be specified in detail and handed to SDT-HF, never written directly, per this actor's own charter §5. **SE's verification that either option preserves DOM-3 is the named precondition before any implementation.** Historical-status correction (item 3) remains held until `F-034` is resolved; no additional `F-033` implementation performed. **No HistFinTS or `histfints_uiue` file modified — read-only throughout.** |
| 2026-09-04 | **Implemented item 1 of DOM-2's binding ordering — F-033 Workbench quarantine integration — `ACTION_PLAN.md` §15s, §5, §20.** SDT-WB's own implementation work, per SE's relay of item 1's authorization; not a DFA/PO governance ruling and not self-certified as one. §15/§15a–§15r preserved completely unedited. Items 2 (`F-034` cadence correction) and 3 (historical-status correction) explicitly **not** implemented. **`histfints.observation_quarantine_active(observation_id)` consumed directly as the complete predicate, no case/run/provenance-mode logic reproduced Workbench-side**: a new shared module, `src/hf_reswb/application/quarantine.py`, is the single place the view is queried. **Consumer-by-consumer disposition**: `panel_eligibility_service.py`/`data_constraints.py` — no integration needed, confirmed by grep and by a dedicated assertion test to hold zero `FROM histfints.observation` reads; `suitability_service.classify_series` — integrated, a quarantined row receives no `ObservationSuitability` record of any kind and the `prior`-close continuity tracker resets to `None` across it rather than comparing across the gap; `reconciliation_service.reconcile` (F-009) — integrated, fetched rows are segmented into maximal contiguous runs at every quarantined observation before `detect_boundaries()` runs, so the two surviving rows on either side of a gap are never treated as newly adjacent; a period wholly quarantined records a new, narrow `ReasonCode.ALL_OBSERVATIONS_QUARANTINED` rather than misusing `NO_CAPTURE_RUN_FOR_SERIES`; `calibration_utilities.py`'s `compute_staleness_lengths`/`compute_panel_depth_by_date` — integrated (found via grep, not named in the instruction), simple SQL-level exclusion, correct here since neither function is a step/persistence comparison across the excluded row. **Continuity treatment recorded explicitly as a conservative default, not a domain ruling**: the `prior`-reset and segment-split both refuse to silently bridge a quarantined gap per instruction, but which specific treatment is correct was not specified anywhere and is not decided by this record — a future dependency to raise, not settled here. **Graceful degradation**: `quarantine.py` treats the view's absence (a HistFinTS copy older than migration 0027 — this repository's own older, deliberately-unbumped shared test fixtures only, never today's actual production schema, already at `user_version=27`) as "nothing quarantined" rather than raising, the same treatment this codebase already gives `provider_event`/`observation_correction`'s absence. A new, isolated `histfints_copy_v27` fixture was added to `tests/conftest.py`, following this project's own established `histfints_copy_v17` precedent of isolating a migration-specific fixture rather than bumping the shared constant. **Regression coverage**: `tests/test_f033_quarantine_integration.py`, 7 new tests, proving quarantined-but-observed-looking exclusion, the no-bridge continuity property (constructed so the naive and correct behaviors disagree), partial-Series eligibility (quarantine binds to `observation.id`, never `series_id`), calibration-input exclusion, F-009 never citing a quarantined observation as evidence, unchanged behavior for a clean series, and the two zero-integration determinations asserted against actual source. **Test results**: new suite 7/7 PASS; full suite 225 passed, 1 skipped, 1 pre-existing failure (`test_ground_truth_against_real_production_series_11312`) confirmed via `git status`/diff scope to be entirely outside this increment's changed files — the already-documented `F-034` defect (§15r), unrelated to and not newly introduced by this work. **Read-only production validation, re-verified directly rather than trusted from SDT-HF's report**: `observation_quarantine_member` COUNT `39876` (`39876` distinct `observation_id`), `observation_quarantine_case` COUNT `24`, distinct `series_id` `17`, distinct `originating_import_run_id` `17` — all four match exactly; a direct read-only script against production (series `11324`, window `2026-08-10`..`2026-08-25`) confirmed `quarantined_observation_ids()` correctly flags 7 of 38 real observations and that `reconcile()` cites none of them as evidence. **One real, already-known interaction, not a new gap**: `classify_series` cannot be exercised directly against real production for any of the 17 currently-quarantined Series today because all 17 currently have `configured_interval='1h'`, independently blocked by the still-unresolved `F-034` defect (§15r) before quarantine is ever consulted — item 2, explicitly out of scope here; quarantine-integration correctness for `classify_series` is proven against the `histfints_copy_v27` fixture instead. **No HistFinTS or `histfints_uiue` file modified — read-only throughout, confirmed by `git status` in both repositories.** |
...
| 2026-09-04 | **Recorded the `F-033` scope amendment — second affected-population subtype confirmed by DFA; PO Option A ACCEPTED, remediation scope expanded to 37,036 observations across 14 Series — `ACTION_PLAN.md` §15n, §5, §20.** Per SE/PO directive, relaying DFA's ruling and PO's subsequent decision — attributed to each respectively, not self-certified by SDT-WB. §15/§15a–§15m preserved completely unedited. **§15m itself preserved as historical truth at the time it was written — not rewritten to pretend fourteen affected Series were already known there.** This record is an explicit correction only, not a retraction: it corrects the prior *affected-population completeness claim*, not §15m's own governing disposition for the seven Series it already named, which stands unchanged. **DFA's ruling recorded exactly as instructed**: the newly discovered 18,322-row population is a second `F-033` affected-population subtype under the same root incident; `CONFIRMED_SYNTHETIC`/`ANALYSIS-INELIGIBLE` treatment is strictly local to the established contaminated observations/intervals; genuine observations from `2026-05-29` onward are unaffected merely by Series membership. **PO's decision recorded exactly as instructed**: Option A ACCEPTED, expanding remediation scope to 37,036 unique observations across 14 Series — original population 18,714 + newly discovered population 18,322, confirmed overlap zero (`18,714+18,322=37,036` exactly). **Correction of record stated precisely, exactly as instructed**: the fourteen defective runs were complete; the earlier seven-Series population was not. **Preserved explicitly**: no accepted INC-7 result is reopened on current evidence (§15b/§15f/§15g/§15h/§15i all remain exactly as recorded; AAPL and Bradesco remain outside both populations); no whole-Series disqualification, catalog merge/deduplication, identity action, lifecycle change, deletion, fabricated replacement, or G1/G9 action is authorized. **Implementation status recorded exactly as instructed**: Workbench quarantine consumption is NOT implemented yet — dependent on SDT-HF returning the committed canonical HistFinTS SQL contract; SE will issue the downstream implementation instruction only after that prerequisite is verified. This documentation record is explicitly independent of that still-pending HistFinTS-side work, persisted now per instruction. §5's detail-range citation and §20's INC-7 pointer bullet both updated to reflect the expanded scope, superseding (not erasing) the prior 18,714/7-Series figures. **No HistFinTS or `histfints_uiue` file modified.** |
| 2026-09-04 | **Recorded the durable PO decision — `F-033` historical CEDEAR remediation scope: PO ACCEPTED — `ACTION_PLAN.md` §15m, §5.** Per SE/PO directive. Recorded exactly as relayed and attributed to PO — not self-certified or re-derived by SDT-WB. §15/§15a–§15l preserved completely unedited. **Extends the disposition from §15l's own directly re-tested trio (Microsoft, MercadoLibre, QQQ) to the full originally-flagged seven-series `F-033` cohort** — including `MU`, `AMD`, `AMZN`, and `NU`, not independently re-tested by SDT-WB's own §15l retest, per PO's own accepted scope decision. **Governing disposition recorded exactly as instructed**: confirmed synthetic CEDEAR observations for MSFT, MELI, QQQ, MU, AMD, AMZN, and NU are `CONFIRMED SYNTHETIC / ANALYSIS-INELIGIBLE` for financial analysis requiring observed CEDEAR market prices. **Preserved explicitly, exactly as instructed**: synthetic rows remain in HistFinTS as provenance/audit evidence, not deleted or quarantined; unresolved historical gaps are preferred to fabricated replacements, no reconstruction/interpolation/synthetic backfill authorized; `2026-08-18` uses the established timestamp/process boundary rather than whole-date invalidation, consistent with §15l's own finding that the shared-driver signature's numerical form changed at that date, not that everything on/after it is wholesale invalid; genuine post-cutoff observations remain potentially eligible under normal existing evidence rules, not a new blanket temporal exclusion; contaminated dates remain dispersion-calibration-ineligible, consistent with §15k/§15l; no accepted INC-7 result is reopened (§15b/§15f/§15g/§15h/§15i all remain exactly as recorded — AAPL and Bradesco were never members of the flagged seven-series cohort); the old dispersion/CV artifact (§15d) remains non-decision-bearing, not reversed or retroactively justified; calibration remains deferred, unaffected. **Implementation status: not yet complete, pending SDT-HF technical design** — this decision records accepted scope and governing disposition only; the technical mechanism for representing `CONFIRMED SYNTHETIC / ANALYSIS-INELIGIBLE` status remains SDT-HF's own design work, not performed or inferred here. §5's detail-range citation extended to §15a–§15m in the same change. **No HistFinTS or `histfints_uiue` file modified.** |
| 2026-09-04 | **Performed a fresh, bounded, read-only `F-033` re-test for Microsoft, MercadoLibre, and QQQ — `F033_CONFIRMED` for all three — `ACTION_PLAN.md` §15l, §5; `docs/evidence/F033_RETEST_2026-09-04.md`.** Per SE/PO directive. §15/§15a–§15k preserved completely unedited; §15k's own `F033_UNRESOLVED`-pending disposition sharpened to a confirmed finding, not reversed — all three remain calibration-ineligible exactly as §15k's own governing rule already specified. **Method faithful to `F-033`'s own original diagnostic**: daily raw implied FX (`CEDEAR/underlying`, no curated ratio-applicability evidence applied) for Microsoft (`11324→6602`), MercadoLibre (`11326→6319`), QQQ (`11328→8193`), and three unflagged comparators reused from the original filing's own comparator set (Alibaba, Baidu, Banco Bradesco); day-over-day returns; pairwise Pearson correlation over full history and split into deep-history/live (`2026-05-29`-onward) windows. **Correlation findings**: MSFT vs MELI `r=0.9999` (full history), `≈1.0000000000` (deep history, exact to double precision, `n=2,866`), `0.99997` (live window, `n=66`) — indistinguishable from perfect correlation in every window, including the live window containing `2026-09-02` itself; QQQ vs MSFT/MELI `r=0.77`–`0.997`, high but consistently below MSFT/MELI's own near-unity value; comparator baseline (genuinely unrelated companies) `r≈0.32`–`0.79`; flagged trio vs comparator trio `r` near zero (`−0.10` to `+0.16`) in every cross-comparison. **Stronger-than-correlation, definitive finding**: raw `MSFT_implied_FX/MELI_implied_FX` ratio was **exactly `1.0000000000` (10 decimal places) on all 54 consecutive trading dates, `2026-05-29`–`2026-08-17`** — impossible for two genuinely independent, differently-priced, differently-ratioed (`30:1` vs `120:1`) companies to produce by any market mechanism. **New, precisely dated transition, not previously in this repository's records**: on `2026-08-18` — the exact date `7f7f73c` populated GATE 1's CEDEAR relationships and produced the primary dispersion/CV evidence later downgraded at §15d — this ratio abruptly shifted to a new, still tightly-bounded constant (`~4.00`, range `3.98`–`4.01` through `2026-09-02`); date coincidence reported, causal link not determined. **Lineage check**: no shared/reused `import_run_id` found across any of the three pairs' six legs — rules out simple storage-layer row reuse, consistent with `F-033`'s own original diagnosis of a shared upstream input or acquisition-side transformation. **Dispositions recorded exactly as instructed**: Microsoft `F033_CONFIRMED`; MercadoLibre `F033_CONFIRMED`; QQQ `F033_CONFIRMED`; three-member group `F033_CONFIRMED`. **Immediate calibration consequence**: none of the three qualifies for reconsideration — `F033_CONFIRMED` (not `F033_CLEARED`) means all three remain calibration-ineligible. **Exact HistFinTS question for a subsequent SDT-HF handoff recorded, evidence-finding stopped here rather than root-caused further**: what acquisition/transformation mechanism produced the exact `1.0000000000` ratio for 54 consecutive dates, why it shifted to `~4.00` exactly on `2026-08-18`, whether the mechanism is upstream-shared-input or acquisition-side, and whether it also touches the four other originally-flagged `F-033` cohort members (`MU`, `AMD`, `AMZN`, `NU`) not re-tested in this pass — full text: `F033_RETEST_2026-09-04.md` §10. **No pair or date added to the dispersion-methodology study. No threshold computed. The provisional median/`LOG_RELATIVE` design (§15k) unchanged. Calibration not performed.** §5's detail-range citation extended to §15a–§15l in the same change. **No HistFinTS or `histfints_uiue` file modified — read-only throughout, confirmed by `git status` in both repositories.** |
| 2026-09-04 | **Recorded DFA's cross-sectional dispersion methodology rulings and performed the next bounded design increment — `ACTION_PLAN.md` §15k, §5; `docs/evidence/CROSS_SECTIONAL_DISPERSION_METHODOLOGY_STUDY_2026-09-04.md` §12 (additive amendment).** Per SE/PO directive, relaying DFA's rulings. §15/§15a–§15j preserved completely unedited; the study document's §1–§11 preserved exactly as originally written, a new §12 added. Used only the same five §15h `2026-09-02` observations — no new pair, no new date, `F-033` not retested. **Governing design choice recorded exactly as instructed**: `LOG_RELATIVE` (`ln(value_i/median)`) is now the primary residual representation, sign/dimensionless interpretation preserved; `PERCENTAGE_RELATIVE` no longer co-equal, retained only as a diagnostic cross-check; the provisional same-date median (`1591.7233371077455`, MSFT's own value) remains accepted for methodology design only, not an approved calibration/production center. **`LOG_RELATIVE` residuals, exact, independently computed**: AAPL `−0.0000897137354274` (below); Bradesco `+0.0006470538945903` (above); MSFT `0.0000000000000000` (at center, structural zero per odd-`n` median-centering, not a correctness signal); MELI `−0.0013535304619091` (below); QQQ `+0.0009616621903794` (above). `PERCENTAGE_RELATIVE` diagnostic cross-check confirms close agreement (largest difference ≈`9.16×10⁻⁵` percentage points). **Inversion-property demonstrated concretely**: reversing the quote convention (`1/value`, `1/median`) flips every pair's `LOG_RELATIVE` sign while preserving magnitude to full IEEE double-precision (residual sums ~`10⁻¹⁷`). **Cohort-membership sensitivity restated as already relevant**: leave-one-out median shifts unchanged from the prior entry; median-member exact-zero structural effect applies identically under `LOG_RELATIVE`. **Calibration-eligibility table recorded exactly as instructed, separated from methodology-design membership**: AAPL and Banco Bradesco — methodology example permitted, no `F-033` exclusion currently identified; Microsoft, MercadoLibre, and QQQ — methodology example permitted, **CALIBRATION-INELIGIBLE while `F-033` unresolved**. Common Yahoo provider usage alone recorded as not a disqualifier; the specific unresolved `F-033` shared-process/deep-history evidence is the calibration concern. **One new methodological ambiguity flagged for DFA review**: the provisional median's own quote-convention invariance (`median(1/values)=1/median(values)`) holds for the median specifically at odd `n` (monotonic-transform commutation) but is not guaranteed for every future robust-center candidate (e.g. a trimmed mean) — named, not resolved. **Explicitly not computed or adopted**: operating dispersion threshold; suppression rule; consensus/panel FX; production center; calibration statistic; raw-price CV; `P90 CV 0.167`; CCL; fair value/mispricing; arbitrage; recommendation; trade/execution logic. **No pair or date added. `F-033` not retested.** §5's detail-range citation extended to §15a–§15k in the same change. **No HistFinTS or `histfints_uiue` file modified — read-only throughout.** |
| 2026-09-04 | **Recorded PO's cross-sectional dispersion methodology-design reactivation and delivered the bounded read-only methodology-design study — `ACTION_PLAN.md` §15j, §5; `docs/evidence/CROSS_SECTIONAL_DISPERSION_METHODOLOGY_STUDY_2026-09-04.md`.** Per SE/PO directive. §15/§15a–§15i preserved completely unedited; §15d's own artifact downgrade not reversed. **PO decision recorded exactly as instructed**: Cross-sectional dispersion — METHODOLOGY DESIGN REACTIVATED / PO ACCEPTED. **Preserved separately, exactly as instructed**: calibration, operating thresholds, suppression rules, consensus/panel output, and production use remain DEFERRED. **Study performed using only the five already-established §15h `2026-09-02` observations, no new pair/date/curation**: two candidate normalized-residual definitions compared (percentage-relative, log-relative), both centered on the same-date median (`1591.7233371077455`, MSFT's own value); both well-behaved, numerically near-indistinguishable at observed magnitudes (largest difference ~9.2×10⁻⁵ pp); neither promoted, neither rejected. **Structural artifact named**: at odd `n=5` the median-as-center is always exactly one member's own value, giving it a residual of exactly zero by construction — not a correctness signal. **Independence diagnostic, the study's single most material finding**: all ten legs trace to the same live Yahoo Finance provider for `2026-09-02`; **three of the five selected pairs — Microsoft, MercadoLibre, QQQ — are members of the still-`LIVE`/unresolved `F-033` shared-driver defect** (deep-history day-over-day return correlation of exactly `+1.00`, diagnosed as likely shared-process/single-input construction, no closure entry found in this repository); today's specific observations confirmed live-provider-sourced (not the flagged `BACKFILL_` mechanism) with instrument-distinct price levels, but `F-033`'s own return-correlation concern was **not** re-tested this pass and is **not** cleared by this study — named as the single most consequential open item. **Sensitivity**: leave-one-out median shifts by at most ~0.032% of center, no disproportionate single-pair influence found. **Smallest candidate set for DFA's selection**: both candidates together, already exposing the one real trade-off (symmetry under inversion) at this dataset's scale. **Exact evidence still missing before calibration**: a fresh `F-033` correlation re-test for MSFT/MELI/QQQ; a wider evidence-qualified population across dates/regimes; documentation of the shared-provider concentration; DFA's own candidate selection; a settled robust-center definition for `n` other than 5. **Explicitly not computed or adopted**: historical raw-price CV methodology; `P90 CV 0.167`; operating threshold; PASS/FAIL classification; suppression rule; consensus FX; representative-panel conclusion; CCL; fair value/mispricing; arbitrage; recommendation; trade/execution meaning. **No production functionality or UI implemented. Calibration not reactivated.** Low residuals not claimed to establish correctness; high residuals not claimed to identify an erroneous pair. §5's detail-range citation extended to §15a–§15j in the same change. **No HistFinTS or `histfints_uiue` file modified — read-only throughout.** |
| 2026-09-04 | **Recorded the Banco Bradesco four-date sparse temporal comparison — COMPLETE under DFA-approved bounded scope — `ACTION_PLAN.md` §15i, §5.** Per SE/PO directive, relaying DFA's approved bounded scope. Every value independently verified live against the production database before being recorded. §15/§15a–§15h preserved completely unedited — a bounded, dated analytical stage over the accepted Banco Bradesco relationship, performed under INC-7's already-approved, already-closed capability, not itself reopening or extending any broader INC-7 capability. **Accepted relationship**: Banco Bradesco, `11355 → 972`, ratio `1:1`. **Four independently evidenced production points, chronologically, independently reproduced live, matching exactly**: `2022-06-09`, assertion `id=7` (new) → `219.9228732465739`; `2022-11-08`, `id=8` (new) → `304.5706459697319`; `2024-07-08`, `id=1` (pre-existing, §15f) → `1392.3581017966142`; `2026-09-02`, `id=3` (pre-existing, §15h) → `1592.753601174374`. **`id=7`/`id=8` confirmed the only new production assertions for this stage** — table holds exactly 8 rows total, all field-for-field consistent with prior records. **`2022-09-06` and `2024-09-17` confirmed deliberately uncurated** — both independently reproduced live as `UNKNOWN`/blocked. **Governing analytical finding recorded in substance, exactly as instructed**: *"Banco Bradesco produced four independently evidenced pair-specific implied-FX observations on 2022-06-09, 2022-11-08, 2024-07-08, and 2026-09-02. The values may be compared factually across those exact dates, but they are discrete observations and do not establish the path between them."* **Consecutive arithmetic differences recorded as arithmetic only, independently recomputed and confirmed exact**: `2022-06-09→2022-11-08`: `+84.6477727232`; `2022-11-08→2024-07-08`: `+1087.7874558269`; `2024-07-08→2026-09-02`: `+200.3954993778`. **None of the following is claimed or implied**: trend; slope; continuous increase; stability/instability; volatility; regime change; interpolation; historical continuity. **Preserved explicitly, as instructed**: historical coverage remains informational/non-blocking; ratio continuity between the four point dates is not established; the 15-day threshold remains explicitly PROVISIONAL, does not prove freshness; no CCL, fair value/mispricing, arbitrage, recommendation, or trade/execution interpretation; cross-sectional dispersion remains DEFERRED (§15d); AAPL continuity remains `UNRESOLVED` (§15c); G1/G9 remains DEFERRED/ON HOLD (§12q). **Stage disposition**: Banco Bradesco four-date sparse temporal comparison — COMPLETE under DFA-approved bounded scope. **Brief user-log entry added, per this repository's established Question/Answer/Immediate-action closure convention**: *Question* → Is the Banco Bradesco four-date sparse temporal comparison complete and within DFA-approved bounded scope? *Answer* → Yes; four independently evidenced production points were verified, the consecutive arithmetic differences were recorded as arithmetic only, and no trend/continuity/CCL/fair-value/mispricing/arbitrage/recommendation/trade conclusion was drawn. *Immediate action* → Preserve the four points and the four arithmetic differences exactly as recorded; do not characterize them as trend, path, or continuity; do not reopen AAPL continuity, cross-sectional dispersion, or G1/G9; treat this stage as complete. §5's detail-range citation extended to §15a–§15i in the same change; no other index required updating. **No HistFinTS or `histfints_uiue` file modified.** |
| 2026-09-04 | **Recorded the bounded 2026-09-02 five-pair descriptive comparison — COMPLETE under PO-approved scope — `ACTION_PLAN.md` §15h, §5.** Per SE/PO directive, relaying DFA's authorized analytical finding. Every value independently verified live against the production database before being recorded. §15/§15a–§15g preserved completely unedited — a bounded, dated analytical stage performed under INC-7's already-approved, already-closed point-date ratio-applicability capability, not itself reopening or extending any broader INC-7 capability. **Stage disposition**: Bounded 2026-09-02 five-pair descriptive comparison — COMPLETE under PO-approved scope. **Five production-backed results, independently reproduced live, matching exactly**: AAPL `11305→33`, `20:1`, assertion `id=2` (pre-existing, §15g) → `1591.580544066751`; Banco Bradesco `11355→972`, `1:1`, `id=3` → `1592.753601174374`; Microsoft `11324→6602`, `30:1`, `id=4` → `1591.7233371077455`; MercadoLibre `11326→6319`, `120:1`, `id=5` → `1589.5703484805817`; QQQ `11328→8193`, `20:1`, `id=6` → `1593.2547735028038`. All six live assertions independently queried, field-for-field confirmed. **DFA-authorized analytical finding recorded verbatim in substance**: *"On 2026-09-02, each of the five selected, evidence-qualified CEDEAR ↔ underlying relationships produced a valid pair-specific implied-FX estimate through the same accepted INC-7 methodology, using exact-date observations and an adjudicated exact-date conversion ratio. The five estimates may therefore be compared descriptively side by side. No panel, consensus, dispersion, representativeness, or market-rate conclusion is established by this exercise."* **Shared basis recorded exactly as instructed**: exact-date `2026-09-02` observations for both legs; accepted relationship identity; compatible adjustment basis; adjudicated exact-date `POINT` ratio applicability — no `PERIOD`, no legacy interval, no continuity claim, for any of the five. **Historical coverage preserved as informational/non-blocking**; no broader historical completeness inferred. **Explicitly not authorized, recorded exactly as instructed, every item a prohibition not a finding**: that the estimates "agree" or are "tightly clustered"; that they confirm one another; a common/consensus FX rate; representative-panel claims; mean, median, range, CV, dispersion, residual, or any other aggregate methodology; CCL; fair value or mispricing; arbitrage; recommendation; trade/execution interpretation — none computed, implied, or approached anywhere in this record. **Preserved unchanged**: 15-day staleness threshold remains explicitly PROVISIONAL, does not establish freshness; **cross-sectional dispersion NOT reactivated**, §15d's downgrade remains exactly as recorded, dispersion remains DEFERRED; **AAPL ratio continuity remains `UNRESOLVED`** (§15c, unaffected — this stage's AAPL point fact is the same pre-existing `id=2` fact, not a new continuity claim); **G1/G9 remains DEFERRED/ON HOLD** (§12q, untouched). **Brief user-log entry added, per this repository's established Question/Answer/Immediate-action closure convention**: *Question* → Is the bounded 2026-09-02 five-pair CEDEAR comparison complete and within PO-approved INC-7 scope? *Answer* → Yes; DFA authorized the descriptive-comparison finding, five production-backed pair-specific results were independently verified, and no panel/consensus/dispersion/CCL/fair-value/mispricing/arbitrage/recommendation/trade conclusion was drawn. *Immediate action* → Preserve the five results and the DFA finding exactly as recorded; do not reactivate cross-sectional dispersion, do not reopen AAPL continuity or G1/G9; treat this stage as complete, not as an opening for a broader panel capability. §5's detail-range citation extended to §15a–§15h in the same change; no other index required updating. **No HistFinTS or `histfints_uiue` file modified.** |
| 2026-09-04 | **Recorded the AAPL production curation under the closed point-date ratio-applicability capability — `ACTION_PLAN.md` §15g, §5.** Per SE/PO directive. Verified directly against the live production database before being recorded, not taken on the relayed summary. §15/§15a–§15f preserved completely unedited — this record does not reopen §15f's closure, only records a subsequent production curation performed under the already-closed capability. **Recorded exactly as instructed, confirmed live**: AAPL pair `11305 → 33`; production `RatioApplicabilityAssertion id=2`; type `POINT`; ratio `20:1`; date `2026-09-02`; source `"BYMA CEDEAR ratio table, dated 2026-09-02: AAPL 20:1"`; stored adjudication reference `DFA-2026-09-04-aapl-point-2026-09-02` — **provenance accuracy note, stated explicitly per instruction**: this reference string was assigned *during curation* as the identifier for DFA's actual 2026-09-04 ruling; it did not exist as a literal string before that write, and is not described here as pre-existing. **Normal production result, independently reproduced live**: pair-specific implied FX `1591.580544066751` (≈ `1591.58`) for `2026-09-02`. `2026-09-01` and `2026-09-03` remain `UNKNOWN`, independently reproduced, each blocking independently, no continuity inferred. **`2024-01-26` remains calculation-ineligible because the CEDEAR observation is absent** — independently reproduced live (`"no observation exists for the CEDEAR leg on or before the calculation date"`), a point-availability blocker distinct from and prior to the ratio-applicability gate, named precisely rather than conflated. **Bradesco acceptance case confirmed unchanged**: assertion `id=1` and its `2024-07-08` result (`1392.3581017966142`) independently re-verified identical to §15f's own record. **Preserved explicitly, as instructed**: AAPL ratio continuity between established dates remains `UNRESOLVED` (§15c unaffected — `10:1`/`2024-01-25`, `20:1`/`2024-01-26`, and this new `20:1`/`2026-09-02` point fact remain three separate facts, no interval curated); this point fact does not establish a historical interval; the 15-day threshold remains explicitly PROVISIONAL and does not prove freshness; no dispersion/consensus, CCL, fair value/mispricing, arbitrage, recommendation, trade/execution, or global validity/liquidity/freshness/eligibility conclusion is authorized. **Explicitly not reopened**: §15c's continuity finding; §15d's dispersion/CV downgrade (cross-sectional dispersion remains DEFERRED); G1/G9 (§12q, remains DEFERRED/ON HOLD); the `2024-01-26` CEDEAR-observation gap itself (named precisely, not treated as a new finding requiring action). §5's detail-range citation extended to §15a–§15g in the same change; no other index required updating for this production-only curation record. **No HistFinTS or `histfints_uiue` file modified.** |
| 2026-09-04 | **Recorded the durable closure of the point-date ratio-applicability capability — CLOSED/PO ACCEPTED — `ACTION_PLAN.md` §15f, §8, §5, §20.** Per SE/PO directive. Gates recorded exactly as relayed and attributed to their owning authorities — not self-certified by SDT-WB. §15/§15a–§15e preserved completely unedited. **Accepted references recorded exactly as instructed**: HistFinTS `0077a67`; UIUX contract `075@18494ea`; Gate B record `077@34ef247` (verified directly, `histfints_uiue` `HEAD` matches — PASS, all 38 `AC-RA-01..38`, no regression); Gate A PASS against `0077a67` (SDT-WB's own three independent passes this session across `daa0152e`→`4cb3091`→`0077a67`, full suite 1816/1816, focused 116/116, both independently re-run); Gate C PASS (DFA); PO acceptance ACCEPTED. **First production acceptance case recorded exactly as instructed**: Banco Bradesco, Series `11355 → 972`, `POINT`, ratio `1:1`, date `2024-07-08`, pair-specific implied FX `1392.3581017966142` (≈ `1392.36`) — independently re-verified live against the production database, unchanged across every commit in the chain; `2024-07-07` and `2024-07-09` remain `UNKNOWN`, each blocking independently, no continuity inferred. **Attached limitations preserved exactly as instructed, none discharged**: the 15-day staleness threshold remains explicitly PROVISIONAL; being within it does not establish freshness; no inferred ratio continuity or historical reconstruction is authorized; no dispersion/consensus, CCL, fair value/mispricing, arbitrage, recommendation, trade/execution, or global validity/liquidity/freshness/eligibility claims of any kind are authorized. **Explicitly not reopened**: AAPL's own §15c continuity finding (still `UNRESOLVED`); the §15d dispersion/CV historical-artifact downgrade (cross-sectional dispersion still DEFERRED); the §12q G1/G9 DEFERRED/ON HOLD decision; no other deferred capability. **No established `nnn_user_log.md` file convention exists anywhere in this repository** (checked directly — no such file, no such naming pattern found); the incremental log entry is instead recorded via this repository's own established `DECISIONS.md` §6 changelog + embedded Question/Answer/Immediate-action block convention, matching every closure record so far this session, not a new file invented for this occasion: *Question* → Is the point-date ratio-applicability capability ready for durable closure? *Answer* → Yes; PO ACCEPTED after Gate A PASS (three-pass delta review), Gate B PASS (`076` FAIL → `077` PASS, all 38 criteria), and DFA Gate C PASS. *Immediate action* → Preserve the Banco Bradesco production acceptance case and every attached limitation exactly as recorded; do not reopen AAPL continuity, dispersion, G1/G9, or any other deferred capability; proceed only to a separately authorized next increment. **New §8 reusable-baseline entry added**, following the established style. §5/§20 updated to match, in the same change — documentation/index consistency validated (no established `nnn_user_log.md` convention found to also update). **No HistFinTS or `histfints_uiue` file modified.** |
...
| 2026-09-02 | **Corrected the stale G1/G9 status — `ACTION_PLAN.md` §12q, §20.** Per SE/PO directive. **§12p preserved unedited as the historical DFA-recommendation stage** — this is the correction of record, not a retrofit, per documentation-lifecycle discipline. **PO ACCEPTED Option B**, recorded exactly as instructed and attributed to its actual owning authority (PO) — not self-certified by SDT-WB. **G1/G9 is DEFERRED/ON HOLD — now settled, not pending.** No G1/G9 implementation is authorized on either repository. No automatic routing of current `EvidenceSignal`s into `IdentityEvidenceEvaluator` is authorized — G1/G9 remains its own Tier 1–4 capability with its own `IdentityEvidenceEvaluator`/`FinancialIdentityConclusion` semantics, never merged with or fed automatically from the accepted Tier 0/1/2 `EvidenceSignal` + human-adjudication architecture. **Formal retirement is not authorized** — consistent with DFA's own stated reason at §12p (a future read-only, reproducible machine-conclusion use case remains plausible); PO's acceptance of Option B does not itself authorize Option C. **Reactivation requires, exactly as instructed, all three**: a concrete product/research need for system-produced `FinancialIdentityConclusion`s, sufficient authoritative inputs, and approved methodology. **Current status pointer/master table updated to reflect the accepted state**, not "pending PO decision" — §20's INC-4 pointer bullet corrected in the same change. **No HistFinTS or `histfints_uiue` file modified.** Existing accepted capabilities (`MatchCandidate → EvidenceSignal`, `IdentityAdjudication`, INC-17) and the INC-7 bounded surface acceptance (§15a) preserved unchanged, none reopened. |
| 2026-09-02 | **Bundled and persisted the completed first INC-7 product decision — `ACTION_PLAN.md` §15a, §5, §20.** Per SE/PO directive. Gates recorded exactly as relayed and attributed to their owning authorities (DFA, PO) — not self-certified by SDT-WB. **Recorded exactly as instructed**: DFA Domain Review Gate PASS WITH PROVISIONAL LIMITATION (Phase 5.3's previously-outstanding gate, named in the prior read-only next-stage assessment); PO ACCEPTED the AAPL CEDEAR↔underlying single-pair implied-FX/staleness diagnostic (`11305 AAPL CEDEAR → 33 AAPL`, already fully calibrated per `docs/calibration-evidence-2026-08-18.md`/`.json`) as the first INC-7 surface. **`15 days` recorded as explicitly provisional and asymmetric**: exceeding it blocks the bounded contemporaneous diagnostic (time-local exclusion, D-042); being within it does not prove freshness — an exclusion threshold, not a positive guarantee. **Historical `P90 CV 0.167` explicitly not authorized for operating dispersion suppression** — calibration evidence only, requires future normalized-dispersion recalibration, not activated by this record. **This bounded increment contains no cross-sectional dispersion/consensus feature** — a single pair has no cross-section to suppress (D-024's pair-panel/cross-section-panel distinction). **Conversion ratio must be known and applicable for the calculation date** — a per-date precondition, not a one-time check (F-021's AAPL ratio-step lesson). **Adjustment-basis/coverage evidence must be compatible** — `provider.adjustment_basis`/`adjustment_basis_override` and availability-marker coverage checks (`data_constraints.py`) remain live preconditions, not bypassed. **Result is pair-specific implied FX only** — explicitly no global eligibility, CCL, fair-value, mispricing, arbitrage, recommendation, or trade interpretation (SP-2, SP-11). **Stale `ACTION_PLAN.md` "blocked by Q-027/Tranche 2" framing superseded for this bounded use case only** — Q-027 substantially resolved by D-037 (narrowed to BYMA pre-2015), Tranche 2 fully deployed/validated by D-044/D-045; INC-7 overall remains BLOCKED per-analysis for every other direction, unchanged. **G1/G9 remains separately DEFERRED/ON HOLD** (§12p), untouched, unrelated, not extended or narrowed by this record. **Next dependency set exactly as instructed**: PO surface acceptance ✓ → UIUX contract ACTIVE → implementation → validation/conformance → DFA Gate C → PO acceptance. **No implementation occurred or is authorized beyond the pre-existing, previously-calibrated Phase 1–5 mechanism** — the next concrete step is a UIUX contract, not further HistFinTS/Workbench implementation. §5 and §20 updated to match, in the same change. **No HistFinTS or `histfints_uiue` file modified.** |
| 2026-09-02 | **Persisted DFA's G1/G9 decision brief — `ACTION_PLAN.md` §12p, §20.** Per SE/PO directive. **Recorded as a DFA recommendation relayed via SE/PO — not a settled decision**; the G1/G9 boundary remains explicitly undecided by this record, per this repository's standing discipline against rounding a recommendation up to a decision SDT-WB has no authority to make and PO has not yet made. **Recorded exactly as instructed**: DFA recommends Option B (DEFERRED/ON HOLD), stated as a recommendation, not the settled outcome — Options A (reactivate) and C (formally retire/supersede) remain live pending PO's selection. **G1/G9 remains a separate capability** with its own Tier 1–4 methodology and its own `IdentityEvidenceEvaluator`/`FinancialIdentityConclusion` semantics, distinct from the accepted Tier 0/1/2 `EvidenceSignal` + human-adjudication architecture. **It must not be merged with or fed automatically from that architecture** — consistent with `IdentityAdjudication`'s own closed baseline ("never cast to or from") and §12/§20's pre-existing G1/G9 boundary note. **No current implementation work is justified**, on either repository, pending PO's decision — consistent with §12o's own standing hold. **Reactivation (Option A) would require, per DFA's brief, all three**: a concrete product/research need for system-produced financial-identity conclusions, sufficient authoritative inputs, and an approved methodology. **Formal retirement (Option C) not recommended yet** — DFA's stated reason: a future read-only, reproducible machine-conclusion use case remains plausible. **Current status remains pending PO selection of Option A/B/C** — not selected on PO's behalf by this record. §20's INC-4 pointer bullet updated to reflect the recommendation and its pending status. §5 unaffected. **No HistFinTS or `histfints_uiue` file modified.** Existing accepted capabilities (`MatchCandidate → EvidenceSignal`, `IdentityAdjudication`, INC-17) preserved unchanged, none reopened. |
| 2026-09-02 | **Persisted the post-INC-17 next-stage plan — `ACTION_PLAN.md` §12o, §20.** Per SE/PO directive. State/plan record — no new artifact to verify against; existing accepted capabilities preserved unchanged, none reopened. **Recorded exactly as instructed**: INC-17 is CLOSED/ACCEPTED and pushed (`508e348`, range `1792572..508e348`, local/remote `HEAD` confirmed matching); the Tier 0/1/2 methodology/documentation gap remains resolved (2026-09-01, unaffected/unextended by INC-17's closure); **real production adjudication remains blocked by zero naturally occurring Tier 0/1/2 `EvidenceSignal` rows** — the pre-existing condition named throughout §12/§12a, unrelated to and not resolved by INC-17; **Tier 3 remains deferred**, out of scope, unaffected; **the next active INC-4 question is the separate, on-hold G1/G9 `IdentityEvidenceEvaluator` boundary** (`057`'s own §0 boundary, referenced throughout INC-4's and INC-17's closure scopes as explicitly untouched); **DFA is now reviewing whether G1/G9 should be reactivated, remain deferred, or be formally retired/superseded** — a live, open review, not anticipated or narrowed by this record; **no new HistFinTS implementation is authorized until that product/domain decision is settled** — a standing hold, binding until DFA's review concludes and PO/SE convey the settled decision. **The BYMA/Event Log Readers and acquisition-monitoring threads recorded as independent operational work**, explicitly not blocking and not blocked by this INC-4/G1-G9 decision. **Existing accepted capabilities preserved unchanged**: both prior INC-4 bounded closures (`MatchCandidate → EvidenceSignal`, `IdentityAdjudication`, 2026-09-01) and INC-17's own closure (§12n, 2026-09-02) stand exactly as recorded, none reopened, restated, or silently extended. §20's INC-4 pointer bullet updated to reflect all three now-closed bounded capabilities and this next-stage plan. §5 unaffected (INC-4's own row is unchanged — INC-4 remains ACTIVE overall, only INC-17's row reflects its own closure, recorded in the prior entry). **No HistFinTS or `histfints_uiue` file modified.** |
| 2026-09-02 | **Recorded Gate C PASS (DFA), Gate D PASS/PO ACCEPTED — INC-17 CLOSED/ACCEPTED — `ACTION_PLAN.md` §12n, §8, §5.** Per SE/PO directive. Gate C (DFA) and Gate D (PO) recorded exactly as relayed and correctly attributed to their owning authorities — never self-certified by SDT-WB, per this repository's standing discipline (§12n states this explicitly). **Final gate state**: Gate A PASS (`0b111d0`, §12g); Gate B PASS (the `069`–`072` chain, §12h–§12m); Gate C PASS (DFA); Gate D PASS/PO ACCEPTED. **INC-17: CLOSED/ACCEPTED.** **Governing boundary recorded exactly as instructed**: *"candidate context + EvidenceSignals → explicit materiality assessment → human financial-identity adjudication → separately authorized catalog action"* — four genuinely separate, separately-authorized steps; context discovery triggers a dimension into the relevant set, never supplies evidence for it (§12b's DFA ruling, unchanged throughout this entire increment). **New §8 reusable-baseline entry added** for INC-17, following the identical style and rigor of every other closed-increment baseline in that section, naming its own governing boundary and closure scope. **Explicit non-extensions recorded, exactly as instructed**: does not close INC-4 overall (its two prior bounded capabilities — `MatchCandidate → EvidenceSignal` and `IdentityAdjudication` itself, both closed 2026-09-01 — remain separately CLOSED/ACCEPTED, unchanged, not reopened); does not authorize Tier 3 processing; does not authorize the separate on-hold G1/G9 capability; does not authorize automated (non-human) adjudication; does not authorize real production adjudication without eligible evidence — every DFA validity gate remains enforced through the one shared `_validation_failures()` code path, unrelaxed. **Full historical chain preserved and cited in full**: DFA §7 ruling (§12b) → SDT-HF design (§12c) → UIUX contract `068` (§12d) → DFA trigger-map ruling (§12e) → operational hold cleared (§12f) → implementation + Gate A PASS `0b111d0` (§12g) → Gate B FAIL/open per `069` (§12h) → corrective fix `2f7e1d8` (§12i) → scoped revalidation per `070` (§12j) → AC-COR-03 fix `27b6865` (§12k) → AC-COR-07 resolved/AC-COR-03 correction per `071` (§12l) → NVDA confirmation + `071`'s attribution error corrected, Gate B PASS per `072` (§12m) → **Gate C/D PASS, CLOSED/ACCEPTED (this record, §12n)**. **All prior stage records (§12a–§12m) preserved completely unedited**, including `069`–`072` and `070`/`071`'s own text; the prior CLOSED/ACCEPTED INC-4 `IdentityAdjudication` baseline (§12, 2026-09-01) preserved unchanged, not restated or reopened. §5 master row updated to CLOSED/ACCEPTED. **No HistFinTS or `histfints_uiue` file modified.** |
| 2026-09-02 | **Persisted UIUX `072` — real-NVDA `AC-COR-08`/`09` confirmation, correcting `071`'s own commit attribution; Gate B PASS — `ACTION_PLAN.md` §12m.** Per SE directive. Verified directly against `histfints_uiue/071_INC17_AC_COR_07_Resolution.md` and `072_INC17_AC_COR_08_09_NVDA_Confirmation.md`, not taken on the relayed summary. **Corrected sequence recorded precisely**: `070` correctly identified a real `AC-COR-03` dual-origin presentation defect at `2f7e1d8` (the immediately-following changelog entry's characterization of it as a cache false negative is itself now understood to be wrong). `071` correctly resolved `AC-COR-07` as not independently exercisable (Outcome 2, two independent grounds: persisted-state unreachability via `FOREIGN KEY ... ON DELETE RESTRICT`/`CHECK` with no `delete()` path anywhere, and control-flow dead code via `_resolve_signals()`'s earlier identical guard) — unaffected by the correction below. `071`'s claim that `AC-COR-03` was already fixed in `2f7e1d8` was an **attribution error**: `071` opened with `git log -1` (then printing `2f7e1d8`), but `27b6865` landed from outside this session mid-investigation, and `071` read the moved working tree rather than a `git show 2f7e1d8:...` snapshot. **The actual `AC-COR-03` fix is `histfints@27b6865`** (already recorded at §12k) — confirmed via `git show 2f7e1d8:.../web.py`, which contains only the old two-way inference. **`AC-COR-03` is PASS at `27b6865`** — `071`'s raw-HTTP disclosure re-verification stands (only its commit attribution was wrong); `072`'s own NVDA capture independently reproduces the identical disclosure text at `27b6865`. **`AC-COR-07` remains not an application FAIL**, not requiring manufactured invalid persisted state — `git diff --stat 2f7e1d8..27b6865` confirmed to touch only the dimension-origin code path and its test file. **`072` confirms the remaining `AC-COR-08`/`09` accessibility sub-check PASS**, real NVDA via `Get-SpeechViewerText` exclusively, both required states (`satisfied` — *"Currently satisfied for the entered effective period"*; `missing evidence` — *"No relied-upon evidence for this dimension"*) reached in one continuous reading sweep at `27b6865`, buffer to `13967` chars with no plateau. **Gate B: PASS / fully discharged** — all three previously outstanding items resolved. **Gate C pending DFA** (review request already outstanding; no new instruction to DFA issued). **Gate D remains open. INC-17 remains NOT CLOSED, NOT ACCEPTED** — Gate B's discharge is a gate-level result, not increment closure or PO acceptance. §5 master row updated to match. **`069`–`072` preserved unedited**, per documentation-lifecycle discipline — `072` is the correction of record for `071`'s attribution error, not a retrofit. All prior stage records (§12a–§12l) preserved completely unedited. **No HistFinTS or `histfints_uiue` file modified.** |
| 2026-09-02 | **Persisted UIUX `071` — `AC-COR-07` resolved (unreachable dead code, not an application FAIL), `AC-COR-03` correction supersedes `070`'s FAIL — `ACTION_PLAN.md` §12l.** Per SE directive. Verified directly against `histfints_uiue/071_INC17_AC_COR_07_Resolution.md`. **`AC-COR-07` — Outcome 2, not independently exercisable, confirmed on two independent grounds**: (1) persisted-state reachability — `match_candidate`'s `FOREIGN KEY ... ON DELETE RESTRICT` plus its `CHECK` constraint make the design's named scenario structurally impossible to persist, and no `delete()` method exists anywhere for `SqliteSeriesRepository`/`SqliteProviderSymbolRepository`; (2) control-flow reachability, a stronger independent finding — both callers of `_validation_failures()` invoke `_resolve_signals()` first, which already raises for a nonexistent `candidate_id` before `_validation_failures()`'s own catch clause for the same condition could ever fire; the web route's own existence check precedes its form-state call the same way. **No synthetic/corrupt persisted state was created** to force the branch; no database integrity constraint weakened. **Test coverage assessed, not remediated** — the domain-level building blocks are tested for all-`None` inputs (correct, by design); the specific `_validation_failures()` catch-and-convert branch and its exact reason text are untested at every layer because nothing in the current codebase can reach them; `071` explicitly leaves removal-vs-retention as SE's/DFA's design judgment, not a UIUX finding. **`AC-COR-03` correction, found while sourcing the above, not requested**: `070`'s "FAIL — all applicable origins disclosed" finding is superseded — a false negative caused by a browser-cache artifact in the embedded-browser-pane method, not a real defect in `histfints@2f7e1d8`. Re-verified via raw HTTP (`urllib.request`, no cache) against one confirmed fresh disposable server; the three-way `dimension_origins` disclosure reproduced correctly, already present in `2f7e1d8` before `27b6865`. `070`'s other four findings re-verified via raw HTTP and confirmed to reproduce identically (those had used distinguishing query strings, which is why only the bare-URL AC-COR-03 check was affected). **`070` itself left unedited**, per this repository's documentation-lifecycle discipline — `071` is the correction of record, not a retrofit. **`27b6865` (§12k) not retroactively recast** as proof `AC-COR-03` was a genuine pre-existing defect — the three-way disclosure logic it reuses was already present and functioning in `2f7e1d8`, before `27b6865`. **Gate B state**: all functional/reachability questions now resolved; **only the previously requested real-NVDA per-material-dimension satisfaction/status confirmation remains outstanding** before Gate B can be fully discharged — not itself claimed discharged by this record. Gates C/D remain open. **INC-17: NOT CLOSED, NOT ACCEPTED.** §5 master row updated to match. All prior stage records (§12a–§12k), including `070` itself, preserved completely unedited. **No HistFinTS or `histfints_uiue` file modified.** |
| 2026-09-02 | **Persisted the `AC-COR-03` corrective fix `histfints@27b6865` — `ACTION_PLAN.md` §12k; Gate B still open, three items remain.** Per SE directive. Verified directly, not taken on the relayed summary. **`AC-COR-03` fix confirmed exactly**: `web.py`'s `dimension_origins` now computed from gathered evidence and a new `discover_context_dimensions_for_candidate()` call, yielding `"evidence"`/`"context"`/`"both"` per dimension, replacing the prior two-way inference. **Deduplication confirmed unchanged** — `relevant_dimensions` still the same plain union/`frozenset`, nothing in the diff touches the union logic. **New service method confirmed read-only/additive, reusing the existing discovery mechanism, not a new algorithm**: calls the exact same pre-existing `domain.discover_candidate_context_dimensions()`, shares a common context-lookup helper with `discover_relevant_dimensions_for_candidate()` so both read exactly the same context, only `.get()` calls, no `.save()`; that pre-existing method's own output confirmed unchanged. **`AC-COR-07` behavior confirmed intentionally untouched** — the `ValueError` condition/message byte-identical to the prior commit. **Materiality/satisfaction/contradiction/adjudication/catalog semantics confirmed unchanged**, no `domain/`/`persistence/`/`cli.py` file in the diff. **Focused presentation tests independently re-run**: 56 passed, exactly. **Application identity-adjudication tests independently re-run**: 37 passed, exactly. **Full suite independently re-run**: 1688 passed, 0 failed (up from 1684). **Live database re-checked, unchanged**: schema version unchanged at 24, `match_candidate` unchanged. **Gate A remains PASS.** **Gate B remains open, pending exactly three items, none claimed resolved by this record**: UIUX revalidation of `AC-COR-03` itself; resolution of `AC-COR-07`'s reachability status (SE's/PO's/DFA's to settle, untouched by this commit); the narrow NVDA check of the per-dimension satisfaction output (`070`'s own named gap). Gates C/D remain open. **INC-17: NOT CLOSED, NOT ACCEPTED.** §5 master row updated to match. **`069` and `070` preserved unchanged as historical evidence** — neither edited. All prior stage records (§12a–§12j) preserved completely unedited. **No HistFinTS or `histfints_uiue` file modified.** |
| 2026-09-02 | **Persisted UIUX `070`'s scoped revalidation of `histfints@2f7e1d8` — `ACTION_PLAN.md` §12j; Gate B still open.** Per SE directive. Verified `histfints_uiue/070_INC17_Revalidation_2f7e1d8.md` directly, not taken on the relayed summary — deliberately narrow, `069`'s own already-PASS items not rerun. **All four scoped corrective checks confirmed PASS, real and end-to-end**: `AC-COR-12` exercised through the real UI this time (not `069`'s direct-service-call bypass) — real checkboxes, a real recorded `ContradictionResolution`, real NVDA confirming both fieldsets `Tab`-reachable, closing `069`'s own `AC-COR-21` partial-FAIL; `AC-COR-08`/`09` confirmed live and distinct per state, functionally/visually closing both gaps, with the NVDA confirmation of the specific new paragraph named as not independently captured this pass — a narrower gap, not rounded up; the `NON_MATERIAL` round-trip confirmed on all three required behaviors (accurate message, preserved selection, specific reason coexisting with the generic one). **`AC-COR-03` recorded precisely as a newly confirmed presentation gap, not a regression**: a genuine dual-origin dimension is correctly deduplicated (shown once) but only one origin is disclosed — root-caused directly in the route's own code comment as a deliberate binary-label simplification; `070` states explicitly this is new evidence from independently exercising a scenario `069` left untested, absent from `2f7e1d8`'s own diff. **`AC-COR-07` recorded precisely as unresolved-as-a-validation-reachability-question, not presently an application FAIL**: a real `FOREIGN KEY constraint failed` confirmed the attempted nonexistent-Series state is schema-prevented; `discover_relevant_dimensions_for_candidate()`'s own source confirms its only "could not be established" path fires on a nonexistent `MatchCandidate`, a different already-handled case — `070`'s own framing ("a DFA/SE design question, not a UIUX finding") preserved exactly. **The stale seven-server condition confirmed a validation-harness artifact, not an INC-17 defect** — `070` §0's own account (seven disposable dev-server processes accumulated on one port, resolved by PID before any scoped evidence was captured) read and preserved as written. **Current state recorded exactly, matching `070`'s own "cannot be called fully discharged from this pass alone"**: Gate A PASS (unchanged); Gate B still open, `070` itself declining to assert whether `AC-COR-03`/`07` block it, naming that SE's/PO's call; Gates C/D open; **INC-17 NOT CLOSED, NOT ACCEPTED.** §5 master row updated to match. **`069` and `070` both preserved as historical validation evidence, neither rewritten** — standing side by side, not merged. All prior stage records (§12a–§12i) preserved completely unedited. **No HistFinTS or `histfints_uiue` file modified.** |
| 2026-09-02 | **Persisted the corrective implementation milestone `histfints@2f7e1d8` — `ACTION_PLAN.md` §12i; Gate B still open, pending UIUX revalidation.** Per SE directive. Verified directly, not taken on the relayed summary. **Scope confirmed presentation-layer only, by the diff's own file list** — `web.py`, one template, one test file; zero `domain/`/`application/`/`persistence/`/`cli.py` files touched, structurally confirming "no domain/service/migration/catalog/CLI semantics changed." **`AC-COR-12` fix confirmed exactly**: `render_template()` now passes the single missing `signals` variable `069` root-caused. **`AC-COR-08`/`09` fix confirmed to reuse existing predicates, not invent new financial-validity logic** — the new helper calls `signal_covers_period()` directly and mirrors the service's own gate 2/3 predicates, its own docstring stating this explicitly. **`NON_MATERIAL` round-trip fix confirmed**: the reviewer's raw classification/rationale now survives an invalid submission with an accurate reason, rather than being discarded. **Focused tests independently re-run and confirmed exactly**: `test_web_identity_adjudication.py` + `test_web_identity_adjudication_materiality.py` together reproduce **52 passed** precisely. **Full suite independently re-run**: **1684 passed, 0 failed** (up from 1676). **Live database re-checked, unchanged**: schema version unchanged at 24, the new INC-17 tables still 0 rows, `match_candidate` unchanged — consistent with a presentation-only fix touching no data. **Gate B remains open, not claimed PASS by this record**, matching the fix commit's own message ("Does not claim Gate B PASS or INC-17 closure"). Gates C/D remain open. **INC-17: NOT CLOSED, NOT ACCEPTED.** **`069` preserved exactly as the failed-validation record, not rewritten** — an accurate account of `0b111d0` at the time, distinct from this later, separate corrective commit. §5 master row updated to match. All prior stage records (§12a–§12h) preserved completely unedited. **No HistFinTS or `histfints_uiue` file modified — read-only throughout.** |
| 2026-09-02 | **INC-17 Gate B recorded FAIL/open — UIUX `069`, `ACTION_PLAN.md` §12h.** Per SE directive. Verified `histfints_uiue/069_INC17_AC_COR_Validation_Evidence.md` directly, not taken on the relayed summary — status "Mixed... Not INC-17 closure," no fix implemented for any finding. **Two confirmed application defects recorded precisely**: `AC-COR-12` (severe, root-caused — `identity_adjudication_evidence_page`'s `render_template()` never passes a `signals` variable, so the resolution-recording form's two signal-selection fieldsets silently render zero checkboxes, no error raised; the entire contradiction-resolution recording workflow is unreachable through the shipped UI); `AC-COR-08`/`09` (FAIL — per-`MATERIAL`-dimension satisfaction state shown only in the aggregate bottom-of-page blocking list, not at the dimension itself, so `AC-COR-09`'s required visual distinction from `NON_MATERIAL` is also unmet). **Backend/service logic preserved as confirmed sound when exercised directly** — `069` bypassed only the broken template, calling `record_contradiction_resolution()` directly against the real running service/repository code, confirming `AC-COR-11/13/14/15` sound, matching Gate A's own independent finding (`26c776f`) exactly. **The `NON_MATERIAL` rationale round-trip issue recorded as an adjacent UX defect, not folded into `AC-COR-05`/`06`'s own PASS** — the rationale requirement genuinely holds, but the blocking-reason text inaccurately reads "have not been classified" when a real classification was made (only the rationale was missing), and the radio group silently discards the reviewer's own selection on the round trip. **Recorded as expected to be corrected in the same bounded patch as the other two defects, not yet applied** — `histfints` `HEAD` confirmed unchanged at `0b111d0`, no such patch exists yet. **`AC-COR-03`/`07` preserved exactly as not independently live-tested in `069`** — confirmed via source review only per `069`'s own table, not silently promoted to PASS. Every other AC confirmed PASS, including real-NVDA evidence for `AC-COR-22..25` captured exclusively via the corrected `Get-SpeechViewerText`, buffer lengths up to 13,976 characters with no plateau. **Current state recorded exactly**: Gate A PASS (unchanged); Gate B FAIL/open, pending a bounded UI correction and UIUX revalidation; Gates C/D open; **INC-17 NOT CLOSED, NOT ACCEPTED.** §5 master row updated to match. **`069` not rewritten as if already fixed — stands exactly as written.** All prior stage records (§12a–§12g) preserved completely unedited. **No HistFinTS or `histfints_uiue` file modified.** |
| 2026-09-02 | **INC-17 implementation milestone persisted and independently conformance-reviewed — `histfints@0b111d0`, migration `0024` live, full suite 1676/1676, Gate A technical PASS, no discrepancy found; not validated, not accepted.** Per SE directive: persist first, then independently review — not accepted on the implementation report alone. `ACTION_PLAN.md` §12g. Read the full diff (`identity_adjudication.py`'s domain additions, `identity_adjudication_service.py`'s new gates, migration `0024`'s full SQL, `web.py`/`cli.py`'s shared-discovery call sites) directly, not summarized from the commit message. **Every requested verification point independently confirmed**: `DimensionMateriality`/`ContradictionResolution` and their `__post_init__` rules read directly; `CONTEXT_DIMENSION_TRIGGERS` re-confirmed to contain exactly the ruled mappings (`security_type`→`INSTRUMENT_SUBTYPE`, `base_symbol`→`PROVIDER_IDENTIFIER`, `ADJUSTMENT_BASIS_DIMENSION` via `ProviderAssignment.adjustment_basis_override` reached through `series_repo`, `ISSUER_SECURITY_IDENTITY` explicitly named as a non-trigger gap with derivation sources explicitly excluded); `discover_candidate_context_dimensions()` confirmed structurally incapable of supplying evidence (`frozenset[str]` return, no path to `relied_upon_signal_ids`); `NON_MATERIAL` rationale enforced at both the domain layer and a schema `CHECK` constraint (defense in depth); the candidate-wide authoritative-contradiction gate confirmed to load the candidate's **full** evidence set (not merely caller-supplied relied/contradictory), closing the exact web-only/CLI-absent omission bypass a prior assessment named, with omission/deselection/tier/preference/majority/recency each explicitly named insufficient in the blocking-reason text; `ResolutionKind`'s exactly-four members and the per-kind XOR field enforcement confirmed in source; `UNRESOLVED`'s exemption confirmed unchanged (`_validation_failures()` returns `()` immediately); `preview_adjudication()`/`record_adjudication()` confirmed to share the identical validation call, and `web.py`/`cli.py` confirmed to call the identical `discover_relevant_dimensions_for_candidate()` service method (grepped, no independent parallel computation); migration `0024` confirmed four new tables only, no `UPDATE`/`DELETE`, `0023`'s own tables untouched; live database re-checked directly (not relying on the report): `PRAGMA user_version=24`, all four new tables present at 0 rows, `identity_adjudication`/`evidence_signal` unchanged at 0 rows, `match_candidate` unchanged (4/9), `observation`/`import_run` showing only continued organic growth, `entity_change_log` unchanged; zero `resolve_*`/`reverse_*`/catalog-save calls, zero `identity_evidence.py`/G1-G9 coupling beyond pre-existing separation docstrings, zero Tier 3 references, and every "score"/"rank"/"confidence" occurrence confirmed to be inside a "never a score" disclaimer — all grepped directly. **Gate A: PASS. No discrepancies found.** **Preserved distinctions**: the previously accepted `IdentityAdjudication` capability (INC-4) remains historically CLOSED/ACCEPTED, §12's own record unedited; INC-17 remains a separate corrective increment; implementation is recorded as **complete, not validated or accepted** — Gates B/C/D remain open and unaddressed by this review, and INC-17 closure is not claimed. All prior stage records (§12a–§12f) preserved completely unedited. **No HistFinTS or `histfints_uiue` file modified — read-only throughout** (`git show`, direct read-only SQL, full test-suite re-run). |
| 2026-09-02 | **Persisted PO's Windows GUI scope decision as D-047 (§1) — frozen legacy surface, no parity requirement, reactivation gated on demonstrated Web-architecture inadequacy.** Per SE directive. Placed in `DECISIONS.md` §1 ("Decided") rather than the changelog narrative or a new standalone document, per explicit instruction to prefer an existing suitable record and this section's own stated purpose (durable, binding architectural/product decisions, the same home as D-001 through D-046). Recorded in full substance: frozen status, no new feature development, no parity requirement; the reactivation criterion (a concrete required capability demonstrated materially inadequate in the Web architecture) with its own non-exhaustive examples (browser-performance limits at required workload, genuine offline/local-state need, essential OS/device integration, another concrete deployment/security constraint); the explicit list of what does **not** by itself satisfy the criterion (the GUI's mere existence, anticipated-but-undemonstrated chart complexity, a native-UI preference, a parity desire); and the retirement safeguard (extract reusable non-UI financial/chart-calculation logic before eventual deletion, not the GUI's own current maintenance). **Explicitly recorded as authorizing none of**: GUI deletion or refactor now, a parity backlog, or any change to INC-17's own execution — INC-17 continues exactly as recorded in §12a–§12f, untouched by this entry. **No HistFinTS or `histfints_uiue` file modified** — this is a Workbench-side persisted record of a PO decision about a HistFinTS-owned surface, not a write to that repository. |
| 2026-09-02 | **Recorded the operational hold cleared and INC-17 implementation ACTIVE — `ACTION_PLAN.md` §12f, appended without rewriting §12a–§12e.** Per SE directive. Verified both claims directly rather than accepting them. **Daily Import completion independently corroborated live**: queried `import_run` for `2026-09-02` directly — 11,309 SUCCESS, 77 FAILED, 1 PARTIAL, matching this project's own already-documented baseline pattern for this scheduled task, with the most recent row ended `12:33:29 UTC` and no newer activity since — consistent with a completed run. **SDT-HF's own design finalization against the trigger-map ruling confirmed one commit further than the prior record**: `histfints@3507f68` ("INC-17: apply DFA's final `CONTEXT_DIMENSION_TRIGGERS` ruling"), read in full — every §12e amendment applied precisely, plus concrete specifics `3507f68` itself resolved (`ProviderSymbol.base_symbol`, not `raw_ticker`, is the field mapped to `PROVIDER_IDENTIFIER`; `ProviderAssignment.adjustment_basis_override` confirmed reachable through the existing `series_repo` dependency and mapped to `ADJUSTMENT_BASIS`, answering §12e's own "if available" condition directly rather than leaving it a gap; presence redefined as an explicit `_is_present()` function). The commit's own message states "no known financial-domain ambiguity remains" and implementation is authorized "subject to operational timing" — the condition this record's first point independently clears. Still design-document-only — no production code changed by `3507f68` either. **Both INC-17 pre-implementation dependencies confirmed complete**: UIUX contract `068` and DFA's trigger-map ruling, fully applied. **Dependency state updated**: `DFA methodology ✓ → SDT-HF design ✓ → [UIUX contract ✓ || DFA trigger-map ✓] → implementation ACTIVE → validation → conformance/domain gates → PO acceptance`. **Preserved distinctions stated explicitly**: the previously accepted `IdentityAdjudication` capability remains historically CLOSED/ACCEPTED, §12's own Gate A/B/C/D record unedited; INC-17 remains a separate corrective increment; "implementation ACTIVE" is a state, not a completion claim — this record does not claim implementation has completed, validation has occurred, or INC-17 has been accepted. §5 master row updated to `ACTIVE`. All prior stage records (§12a–§12e) preserved completely unedited. **No HistFinTS or `histfints_uiue` file modified — read-only verification only.** |
| 2026-09-02 | **Persisted DFA's final INC-17 trigger-map ruling — the trigger-map dependency is now COMPLETE; both parallel tracks done; implementation AUTHORIZED, operationally held while the morning Daily Import is active — `ACTION_PLAN.md` §12e, appended without rewriting §12a–§12d.** Per SE directive. Verified the base table being amended directly before recording: `histfints@2f1da5a` (`HEAD` unchanged since the prior record; `b661058` remains the only later commit, the unrelated Daily Import operational checkpoint) — `docs/future_designs/INC17_CORRECTIVE_INCREMENT_DESIGN.md` §1a's `CONTEXT_DIMENSION_TRIGGERS` table re-read in full to confirm exactly what each amendment changes, not approximated against memory. **Amendments recorded precisely against the actual table content**: `ProviderSymbol.security_type` corrected from its current separate `SECURITY_TYPE` mapping to `INSTRUMENT_SUBTYPE`, consolidating it with `Series.instrument_subtype` per the table's own existing "one dimension per concept" convention; a provider identifier field added as a genuinely new trigger for `PROVIDER_IDENTIFIER` — recorded as distinct from `EvidenceSignal`'s own pre-existing `IDENTIFIER` dimension (an evaluated match fact, not a raw-presence trigger) — not conflated; trigger semantics refined from a blind Python `is not None` to explicit typed-value/state presence, extending `EffectiveApplicability.UNKNOWN`'s own established "explicit missing-state, not a null check" precedent; `ISSUER_SECURITY_IDENTITY` confirmed a real, no-existing-typed-field gap, with an explicit prohibition on inferring it from labels — closing a loophole the design's own value-inspection boundary hadn't explicitly named; `ADJUSTMENT_BASIS` ruled to surface only from applicable typed provider/path context when available (tying directly to INC-6's own closed `adjustment_basis`/`adjustment_basis_evidence` fields), otherwise recorded as a standing context-assembly gap, not defaulted; corporate-action/effective-date history ruled to remain evidence/temporal-validation territory (tying to INC-5's closed `ProviderEvent` capability and the governing reference's own §7a/§8), explicitly not folded into candidate-context discovery. **Dependency state updated to reflect all four items complete**: `DFA §7 ruling ✓ → SDT-HF design finalization ✓ → [UIUX contract ✓ || DFA trigger-map ✓] → implementation AUTHORIZED — operationally held (S6, `b661058`) → validation`. **The operational hold named explicitly as distinct from every design/methodology dependency above it** — a standing scheduling checkpoint, lifted by the Daily Import's own completion, not by anything this record settles. **Does not claim implementation has occurred or INC-17 has been accepted.** All prior stage records (§12a–§12d) preserved completely unedited. **No HistFinTS or `histfints_uiue` file modified** — the design document's own trigger table remains SDT-HF's to amend; this record persists DFA's ruling against it without applying the amendment to the sibling repository. |
| 2026-09-02 | **Recorded UIUX `068` as COMPLETE — the second of INC-17's two named milestones, `ACTION_PLAN.md` §12d, appended after `bf32910` without rewriting §12c.** Per SE directive. Verified `histfints_uiue/068_INC17_Materiality_Contradiction_Resolution_UX_Contract.md` directly, not taken on the relayed summary: grounded explicitly in `histfints@2f1da5a`, status "specification work only... Not implemented, not accepted, not closed," 25 numbered acceptance criteria (`AC-COR-01..25`) confirmed by direct count. **Coverage independently checked against the requested list, item by item**: dimension origin/presentation; explicit `MATERIAL`/`NON_MATERIAL` classification with nothing preselected; non-material rationale required, material not; stronger-disposition blocking on an unclassified dimension, `UNRESOLVED` exempt; per-`MATERIAL`-dimension evidence/temporal/satisfaction-state visibility; structured contradiction resolution (contradictions visible regardless of selection state, exactly the four DFA-approved mechanisms, tier/preference/majority/recency excluded as resolvers, acknowledgment kept distinct from resolution, full history discoverable); the five-stage evidence chain preserved at every UI stage; explicit prohibitions on scoring/recommendation/automation; keyboard/NVDA acceptance criteria (drafted, not yet validated). **`AC-COR-25` confirmed to name the corrected `Get-SpeechViewerText` capture path** (`histfints@0a8377a`) as the required future NVDA evidence-capture mechanism, explicitly to avoid repeating the capture-bug-confounded pattern earlier INC-4 validation passes went through. **`CONTEXT_DIMENSION_TRIGGERS`'s concrete field list confirmed deliberately excluded** (§8 of `068`) — the contract governs presentation behavior, not the mapping's contents, so a future DFA amendment to the mapping requires no AC revision. **Explicit correction recorded**: §12c's own closing statement ("UIUX should return the INC-17 contract") is now stale — fulfilled by `068` — corrected in a new record rather than left standing or silently edited into the historical section. §12c itself preserved completely unedited, exactly as instructed. **Dependency state updated**: `DFA §7 ruling ✓ → SDT-HF design finalization ✓ → [UIUX contract ✓ || DFA trigger-map confirmation OPEN] → implementation → validation` — only the UIUX branch checked off; the trigger-map confirmation's status is unchanged and still gates production implementation. §5 master row updated to match. **No new instruction to SDT-HF** — the `CONTEXT_DIMENSION_TRIGGERS` extraction request remains outstanding, unchanged. **Does not claim INC-17 implemented or accepted. No HistFinTS or `histfints_uiue` file modified.** |
| 2026-09-02 | **Recorded INC-17's now-current stage — DFA §7 ruling and SDT-HF design finalization both COMPLETE at `histfints@2f1da5a`; UIUX contract drafting and a narrow DFA trigger-map confirmation open in parallel; production implementation still not authorized.** Per SE directive. Verified `2f1da5a` directly, not taken on the relayed summary: `HEAD` matches (one later commit, `b661058`, confirmed an unrelated operational checkpoint about Daily Import timing, not INC-17 substance); read the design document's own revised §7/§1a/§9 in full. **Confirmed precisely**: neither of the design's own original alternatives ((a) fixed-taxonomy-always-material, (b) scoped-strictly-to-gathered-evidence) matched DFA's ruling, so §7 was rewritten to state the actual rule directly, not approximated to either label — the design's own text says so verbatim. **The settled mechanism, verified against source, not restated from summary**: the relevant-dimension inventory is the union of gathered-evidence dimensions and dimensions mechanically discoverable from the candidate's own financially-typed context fields (a new `CONTEXT_DIMENSION_TRIGGERS` table); context discovery is structurally confirmed never able to supply evidence — `discover_candidate_context_dimensions()` returns only `frozenset[str]` dimension names, with no code path letting its output satisfy a dimension or be passed as relied-upon evidence; the design's own comparison table confirms deterministic discovery reads only field *presence*, never field *value*. **One narrow DFA confirmation remains open, confirmed genuinely narrow**: whether the specific `CONTEXT_DIMENSION_TRIGGERS` field list (built from already-existing typed `Series`/`ProviderSymbol` fields) is complete against DFA's own governing taxonomy — the design's own text frames this as a confirm-or-amend question against a finite table, not a re-litigation of the mechanism. **UIUX contract drafting authorized to proceed independently, in parallel** — the design's own §9 states "ready for the UIUX contract stage," with the trigger-table confirmation explicitly named as not gating it. **Production implementation still not authorized** — the design's own §9, verbatim: "Nothing in this document authorizes starting implementation yet," depending on both parallel tracks completing. **Real production adjudication remains separately blocked** by the continued absence of naturally-occurring Tier 0/1/2 `EvidenceSignal`s — re-verified live at the design document's own writing (still 0/0/0), unchanged and unrelated. **`ACTION_PLAN.md` updated**: §12c added as the current-stage record, explicitly preserving §12b's own prior-stage content unedited — stated directly that §12b describes the state as of the earlier commits and is not being rewritten as if it had never been current; §5 master row updated with the new dependency line (`DFA §7 ruling ✓ → SDT-HF design finalization ✓ → [UIUX contract || DFA trigger-map confirmation] → implementation → validation`), the two bracketed items explicitly parallel, not sequential. **No new instruction issued to SDT-HF or UIUX beyond what was already stated as owed** (the trigger table; the contract), per explicit instruction. **Documentation only. No HistFinTS or `histfints_uiue` file modified.** |
| 2026-09-02 | **Corrected the dependency-order wording recorded at `a3ebe70` — a second DFA/PO review was stated too strongly as an unconditional next step; it is conditional, not mandatory.** Per SE directive. Fixed in place, not by editing the historical changelog entry: `ACTION_PLAN.md` §12b's own dependency-order line and the INC-17 master-table row (the two live spots `a3ebe70` actually introduced) both corrected to: DFA §7 ruling (done) → SDT-HF design mapping/finalization (current step) → **conditional** DFA/PO clarification, **only if** SDT-HF's mapping identifies a remaining domain/product ambiguity the ruling does not settle → UIUX contract → implementation → validation. Explicit note added: if SDT-HF reports the ruling fully settles the design boundary, the conditional step does not occur and the sequence proceeds directly to the UIUX contract — no second DFA/PO review is implied as mandatory in that case. **All other content of §12b preserved unchanged**, per explicit instruction — including the recorded fact that DFA ruled on the underlying methodology rather than selecting a literal internal §7 option label. `§12a`'s own, earlier, differently-scoped dependency-order line (recorded at `0ce075c`, describing the pre-design-proposal stage) left untouched — out of this correction's scope. **Documentation-only, proceeded while the morning Daily Import continued running**, per explicit note. **Does not modify HistFinTS or `histfints_uiue`.** |
| 2026-09-01 | **Persisted DFA's ruling on INC-17's §7 boundary question — the candidate-context completeness rule — `ACTION_PLAN.md` §12b.** Per SE directive. Verified the context directly, not taken on the relayed summary: `histfints@cf5463a` ("Persist INC-17 corrective-increment design (proposed, not implemented)"), read in full — SDT-HF's own technical design, correctly stated `Status: proposed, not implemented`, preserves the already-closed INC-4 manual-adjudication capability (migration `0023`) exactly as historical acceptance, and names one concrete, unresolved boundary question at its own §7: whether materiality review requires (a) an explicit determination against a fixed taxonomy regardless of gathered evidence, or (b) stays scoped strictly to gathered evidence — explicitly declining to choose, since engineering cannot resolve it without inventing financial-methodology taxonomy. **DFA's ruling, relayed by SE/PO, recorded as a synthesis, not a literal pick of (a) or (b)**: a case-specific relevant-dimension inventory, derived from governing identity taxonomy plus financially typed candidate context together; candidate context discovers potentially relevant dimensions but is not itself authoritative evidence; every relevant dimension must be explicitly `MATERIAL` or `NON_MATERIAL` (no silent third state); `NON_MATERIAL` requires reviewer rationale (confirmed already consistent with the design's own `DimensionMateriality.__post_init__`, not a new constraint); every `MATERIAL` dimension must satisfy the existing evidence/temporal-applicability/contradiction gates regardless of its origin; evidence presence/absence alone cannot define the inventory or materiality (rules out both pure (a) and pure (b) as complete answers on their own); inability to establish relevant-set completeness forces `UNRESOLVED`. **Recorded explicitly, not glossed over: DFA could not inspect the literal §7 option labels** — DFA does not read `histfints` source directly, per this project's actor-model routing; the ruling answers the underlying methodological question, not a literal selection between SDT-HF's own internal labels. **SDT-HF is now mapping this ruling against the persisted design before UIUX work begins** — recorded as the current next step, not performed or anticipated here. **Dependency order continued from the prior record, this step inserted precisely where it occurred**: SDT-HF design (named the question) → DFA's ruling (this record) → SDT-HF maps the ruling to the design (current, not yet done) → PO/DFA domain-ambiguity check → UIUX contract → implementation. §5 master row for INC-17 updated to reflect this state precisely. **Does not implement, does not accept, does not itself update SDT-HF's own persisted design document (that remains SDT-HF's to do), does not modify HistFinTS or `histfints_uiue`.** |
| 2026-09-01 | **Persisted the current INC-4/INC-17 corrective-increment plan and dependency state as the record of record, `ACTION_PLAN.md` §12a.** Per SE directive, in addition to the prior stale-state correction. Consolidated, in explicit dependency order, all seven required facts: (1) the governing Tier 0/1/2 methodology is established (`8bcab60`); (2) DFA subsequently identified canonical persisted semantics for case-specific materiality and authoritative-contradiction resolution, incorporated into the governing reference's §6b/§7b/§7c; (3) a read-only HistFinTS assessment (performed while adopting the governing reference, no file modified) confirmed both as real implementation gaps, cited to the exact shipped functions (`_adjudication_form_state()`'s fresh-per-request dimension derivation with no persisted materiality record; `diagnose()`/`_validation_failures()`'s flat-boolean contradiction detection with no resolution-recording mechanism, no correction/supersession record type, no authority-precedence table); (4) the previously accepted `IdentityAdjudication` capability remains historically CLOSED/ACCEPTED, §12's own Gate A/B/C/D record unedited, not reopened; (5) a new bounded corrective increment (INC-17) is now in technical-design stage, not implemented, not accepted; (6) SDT-HF is currently tasked with producing that design before implementation, recorded as the current task assignment per SE/PO's own instruction, not decided by SDT-WB; (7) real production adjudication remains separately blocked by the absence of naturally-occurring Tier 0/1/2 `EvidenceSignal`s, unrelated to and not itself blocking INC-17. **Dependency order preserved explicitly**: governing methodology → DFA's ruling → read-only gap confirmation → SDT-HF design (current stage) → PO/DFA domain-ambiguity check → UIUX contract → implementation, with items 4 and 7 stated as standing facts outside that chain. §5 master-table row for INC-17 unchanged (`NEXT`, already reflecting this sequencing). **Does not claim INC-17 is implemented or accepted. No HistFinTS or `histfints_uiue` file modified.** |
| 2026-09-01 | **Corrected the stale "no SDT-HF task is currently justified" statement recorded at `8bcab60`, and recorded a new bounded increment (INC-17) as now justified for SDT-HF technical design — not implemented, not accepted.** Per SE directive relaying DFA's subsequent canonical adjudication-record ruling: two real, forward-looking implementation gaps in the already-closed `IdentityAdjudication` capability — case-specific materiality persistence and affirmative authoritative-contradiction resolution — both already named in the governing methodology reference's own "Implementation status" notes (§7b/§7c, §6b), not newly discovered here. **Preserved both required distinctions, stated explicitly, not implied**: the previously accepted `IdentityAdjudication` capability remains historically CLOSED/ACCEPTED, unedited, not silently reopened — §12's own Gate A/B/C/D closure record stands exactly as written, INC-17 is a separate, later, corrective piece of work responding to a subsequent ruling, the same posture already established by INC-6's own Finnhub boundary condition; and real production adjudication still waits for naturally-occurring eligible Tier 0/1/2 evidence (`evidence_signal`: 0 rows, unchanged), which is unrelated to and does not block INC-17's technical design/implementation. **`ACTION_PLAN.md` updated**: §5 master row added for `INC-17` (`NEXT`, sequencing `SDT-HF (design) → PO/DFA (domain-ambiguity check) → UIUX (contract) → SDT-HF (implementation)`, per PO's own stated sequence); §12a added as the full justification record, naming both gaps precisely and citing the governing reference's own sections; §20's stale line corrected in place with an explicit "Correction (2026-09-01)" marker rather than silently rewritten. **Does not claim INC-17 is implemented or accepted. No HistFinTS or `histfints_uiue` file modified.** |
| 2026-09-01 | **Incorporated DFA's final rulings into the Tier 0/1/2/3 methodology reference, designated it the governing Tier 0/1/2 methodology reference, and resolved `ACTION_PLAN.md` §1's Tier 0/1/2/3 GAP row for Tier 0/1/2 (Tier 3 remains deferred).** Per SE directive relaying DFA's rulings. **Document renamed in status, not in filename**: "Tier 0/1/2 Financial-Identity Evidence Methodology — Governing Reference," status **GOVERNING**, authority explicitly grounded only in surviving cited rulings (implemented/tested code, `024`'s DFA-D03/D04, and this update's own incorporated DFA rulings) — the unrecoverable `docs/28` provenance limitation preserved verbatim in §1, not claimed away. **Rulings incorporated, each in its own subsection, clearly distinguished from the already-implemented material compiled earlier**: §6b — the authoritative-contradiction resolution rule (four evidentiary acts: correction/supersession; temporal separation; subject/scope disambiguation; established domain-valid authority precedence) and the explicit rule that deselection, unprincipled source preference, majority vote, higher tier alone, or later retrieval alone never resolves a contradiction, plus the required preservation of both conflicting signals and the affirmative resolution evidence/rationale/effective period; §7b — the case-specific material identity dimension rule, and the ruling that `BLOCKING_REQUIRED_DIMENSIONS` is an implementation default, not independent financial authority; §7c — disposition-specific conditions for `SAME_INSTRUMENT` (joint sufficiency, contradiction-freedom, temporal applicability for an identity claim) and `RELATED_BUT_DISTINCT` (a structural relationship plus a material distinction ruling out identity), on equal evidentiary footing; §13 — Tier 3 and G1/G9's separate `Tier 1–4` hierarchy explicitly reaffirmed unaltered by this update. **Each newly-incorporated section carries its own explicit "Implementation status" note stating the shipped `histfints` code does not yet conform** — per explicit instruction, this document's adoption is not itself a conformance claim; the authoritative-contradiction gate (§6a) remains a flat boolean with no resolution-recording mechanism, no correction/supersession record type, and no citable authority-precedence table; the disposition-validity gates (§7a) remain identical for both dispositions rather than distinguishing an identity question from a relationship-plus-distinction question. **`ACTION_PLAN.md` updated**: §1's open-reference row split to `Tier 0/1/2 | RESOLVED`, citing the governing document, with Tier 3 explicitly named as still deferred, not swept into the same resolution; §12 gained a pointer to the governing reference with the same non-conformance caveat restated; §17 item 6 updated to reflect the partial resolution; §20 updated, including the explicit note (per PO) that no SDT-HF task is currently justified — real adjudication waits on a genuinely eligible Tier 0/1/2 signal occurring naturally. **Does not reopen either closed INC-4 capability, does not modify HistFinTS or `histfints_uiue`, does not claim implementation conformance.** |
| 2026-09-01 | **Completed the `docs/28` upstream-source verification for the draft Tier 0/1/2/3 methodology reference — result: the citation does not resolve to any Tier 0–3 content, and the original source is genuinely unrecoverable, not merely unread.** Per SE directive. Read `histfints_uiue/028_Catalog_Discover_Relationship_Presentation_Disposition.md` in full: dated 2026-08-26, entirely a small, unrelated Discover UX presentation-completion disposition (AC-DIS-05/06), §1–§7 only — no Tier definitions, no matching-cascade rules, nothing corresponding to `catalog_matcher.py`'s cited §4-6/§7/§9/§11/§14. Traced why: the commit that first implemented Tier 0/1 (`histfints@891a40d`, 2026-08-10) states "docs/28 -> v4" was genuinely the matching-cascade design document at that date — sixteen days before `028`'s current content was even written. Confirmed unrecoverable, not assumed: `histfints_uiue`'s entire git history begins at one `43abedb` "Initial commit"; the repository was untracked on 2026-08-10; `PROJECT_INDEX.yaml`'s own `catalog_discover` sequence confirms `028` has only ever been this one document, not an overwritten file. **One genuine, additional, directly relevant citation found while verifying, added to the draft rather than left out**: `024_Catalog_Discover_UX_Specification.md`'s **DFA-D03** (Evidence tiers: "Tier 0/1/2 classify strength, quality, or authority of evidence... not confidence percentages, financial conclusions, universal guarantees") and **DFA-D04** (Auto-resolution: "Automatic financial-identity adjudication from Tier 0–2 alone is not established as financially legitimate") — a real, named, independently-checkable DFA ruling, now cited in §9 alongside the domain-code structural evidence already there. **The draft updated accordingly, not harmonized by inference**: §1 replaced with the precise verification result; §9 gained the DFA-D03/D04 citation; §12 rewritten as the requested "finalized list of questions that actually require DFA judgment" — three items reduced to two live open questions (the original item 1, "not yet re-read," is now subsumed by the more precise finding that the source cannot be produced at all) plus the two substantive financial-methodology questions preserved **exactly as instructed, not resolved technically**: (1) which `identity_dimension` values are materially required per disposition, including whether the shipped `BLOCKING_REQUIRED_DIMENSIONS` default has methodological authority or is only an implementation default; (2) what evidence establishes that an authoritative contradiction has itself been resolved. **G1/G9's separate `Tier 1–4` vocabulary re-confirmed kept distinct, not merged** — unaffected by this pass, §0's boundary statement unchanged. **Does not resolve `ACTION_PLAN.md` §1's GAP row, does not ask DFA for approval, does not reopen either closed INC-4 capability, does not change any financial rule.** **No HistFinTS or `histfints_uiue` file modified — read-only throughout.** |
| 2026-09-01 | **Drafted a consolidated Tier 0/1/2/3 financial-identity evidence methodology reference — documentation compilation only, not a new ruling, not yet DFA-reviewed.** Per SE directive. Compiles, with a traceable source for each item, the methodology already implemented and independently verified across the closed `EvidenceSignal` and `IdentityAdjudication` capabilities: Tier 0/1/2 definitions and Tier 3's structural exclusion (`EvidenceSignal.__post_init__` rejects `tier==3`); the quantity-does-not-promote-Tier-2 rule; `SUPPORTS`/`CONTRADICTS`/`INFORMS` stance semantics; the three-clause independence rule (distinct lineage + material dimension + compatible period); derivative-evidence exclusion; authoritative-contradiction handling (both the domain-level blocking gate and UIUX's separate per-signal acknowledgment requirement); the temporal-applicability requirement for material dimensions; `UNKNOWN`/incompatible-period treatment, distinguished from a genuine `KNOWN`-vs-`KNOWN` conflict; the required-explicit-effective-period rule and `UNRESOLVED`'s exemption from every evidentiary gate; the evidence-strength-vs-adjudication/score/recommendation distinction (cited to type-level structural guarantees, not only docstrings); and the three-domain boundary (`EvidenceSignals → human adjudication disposition → separately authorized catalog action`), re-cited to its structural verification already on record. **Explicitly marked OPEN rather than completed by inference**: (1) `docs/28`'s own full text — the cited founding source for Tier 0–3's basic mechanics — was not independently re-read in full this pass, only confirmed to exist with matching section structure; (2) which `identity_dimension` values are materially required per disposition — the shipped default (every dimension the candidate's own evidence touches) is a tested implementation choice filling an explicitly-acknowledged `BLOCKING_REQUIRED_DIMENSIONS` gap, not itself a DFA ruling on per-disposition materiality; (3) how a resolved authoritative contradiction should itself be evidenced — no source found addresses this beyond "the UI only re-evaluates... whatever is currently relied-upon." **Also explicitly named, not silently merged**: the separate, differently-numbered `Tier 1–4` evidence hierarchy in `docs/evidence/G1_G9_Final_Domain_Ruling.md` §2, for the distinct, on-hold, automated `IdentityEvidenceEvaluator` capability — a genuinely different vocabulary for a different capability, not the subject of this compilation. **Does not resolve `ACTION_PLAN.md` §1's Tier 0/1/2/3 GAP row** — that remains open until DFA reviews this draft; not marked resolved here. **Does not reopen either closed INC-4 capability, does not change any financial rule, does not modify HistFinTS or `histfints_uiue`.** **Document:** `TIER_0_1_2_3_FINANCIAL_IDENTITY_EVIDENCE_METHODOLOGY_REFERENCE_2026-09-01.md`, returned to SE for review — DFA approval not yet requested, per explicit instruction. |
| 2026-09-01 | **INC-4's manual financial-identity adjudication capability (`IdentityAdjudication`) recorded CLOSED/ACCEPTED — Gate D (PO) now PASS, Gate B now fully discharged, per PO's direct instruction, attributed to its actual owning authority.** This is the final closure event for this bounded capability. **Verified the "fully discharged" claim directly rather than accepting it on the instruction alone**: read the full `061`–`067` NVDA/Speech Viewer investigation chain (`histfints_uiue`) in full. `061` found a real, genuine application defect (confirm-page transition had no author-specified focus target, reproduced 3/3 times under a confirmed exclusive desktop); `064` tested the resulting fix (`histfints@40eae9e`, independently confirmed via `git show`) and found it did **not** resolve the freeze — strong negative evidence pointing away from the application; `065` then found the real cause: the ad hoc NVDA capture method used throughout `057`–`064` plateaus at ~4096 characters against Speech Viewer's real `RICHEDIT50W` buffer, which was demonstrably current and complete the whole time (4280 real characters vs. 4096 reported) — **the content was never frozen; the capture tool had a length ceiling**; `066` re-confirmed AC-FID-19/20 and the radio sub-check all PASS using the corrected tool (`histfints@0a8377a`, independently confirmed via `git show --stat` to be tooling-only, zero `src/` files touched). **Caught and independently re-verified one genuine documentation error along the way, not silently accepted**: `066` itself misstated AC-FID-04 as still open — checked directly against current `histfints` source and confirmed false (`_temporally_incompatible_ids()`/"Temporal note" intact, `git log 103de84..HEAD` touches zero relevant files) — matching `067`'s own reconciliation of the same error exactly. Full suite re-confirmed at current HEAD (`0a8377a`): **1613 passed, 0 failed**. Live database re-checked: `PRAGMA user_version=23` unchanged, `identity_adjudication` still 0 rows. **`ACTION_PLAN.md` §12 updated**: the top-level state line now lists both closed bounded capabilities; the Gate B section gained a full account of the `061`–`067` chain, explicitly preserving both the freeze findings and the documentation-error finding as unedited historical record, not rewritten, per instruction; Gate D recorded ACCEPT; overall capability state recorded CLOSED/ACCEPTED with an explicit closure-scope statement (does not close INC-4 as a whole, does not authorize automated adjudication or Tier 3 processing, does not extend to G1/G9, does not reopen INC-12/13/15 or the already-closed `EvidenceSignal` sub-capability); §8 gained a baseline entry; §20 updated to match. **The DFA methodological boundary preserved exactly**: `EvidenceSignals → human adjudication disposition → separately authorized catalog action`. **No HistFinTS or `histfints_uiue` file modified — read-only throughout.** |
| 2026-09-01 | **INC-4's manual financial-identity adjudication capability recorded Gate A PASS + Gate B application-level PASS (not fully discharged) + Gate C PASS (DFA) — Gate D open, overall NOT CLOSED/ACCEPTED.** Per SE directive. Preserved the DFA methodological boundary exactly (`EvidenceSignals → human adjudication disposition → separately authorized catalog action`), re-confirmed structurally enforced (`IdentityAdjudicationService` holds `MatchCandidateRepository` read-only, calls no `ResolutionOperation`) rather than merely restated. **`ACTION_PLAN.md` §12 updated**: a new "Gates A/B/C — manual financial-identity adjudication capability" record added, explicitly distinguished from the already-closed `EvidenceSignal` retention capability above it (a second, separate bounded capability inside INC-4, not a reopening or extension of the first). Gate A cites the prior independent conformance review (`7248ffe`) as evidence, not re-derived. **Gate B stated precisely, per explicit instruction — application-level PASS, not manufactured as fully discharged**: every application defect found during validation (`057`'s CSRF/`required_dimensions` gaps, `059`'s AC-FID-04 gap) confirmed resolved through `060`; AC-FID-17/18 confirmed PASS; AC-FID-19/20 and one radio-announcement sub-check remain attempted-not-confirmed, attributed plainly to the disclosed NVDA/shared-desktop validation-environment issue and **explicitly not treated as an application FAIL**, per instruction. **Gate C recorded per SE relaying DFA's ruling, attributed to its actual owning authority.** **Gate D recorded open, not claimed.** **Overall increment state: NOT CLOSED, NOT ACCEPTED** — stated as the section's own explicit conclusion, not left implicit. Does not extend to or reopen the already-closed `EvidenceSignal` capability, INC-12/13/15, or authorize any automatic identity resolution (`SP-1` unaffected). **No HistFinTS or `histfints_uiue` file modified — read-only throughout.** |
| 2026-09-01 | **Independent read-only INC-4 conformance review of the manual financial-identity adjudication capability against `histfints@103de84` and UIUX `056`–`060` — technical PASS, no remaining application-level discrepancy.** Per SE directive. Verified everything directly against real source, the live database, and the full UIUX evidence chain, not the relayed summary. **`histfints`**: `HEAD` matches, working tree clean except unrelated same-day BYMA measurement output; full suite **1609 passed, 0 failed**, matching the implementation's own reported count exactly. Read the full five-commit chain (`503aa91` backend → `7ed62f2` DFA temporal-applicability ruling + preview hook → `9d3f181` Resolve UI integration → `b2a0ef5` two runtime-defect fixes → `103de84` AC-FID-04 disclosure fix). **Every requirement independently confirmed against real source, not restated from the commit messages**: `AdjudicatedDisposition` is its own type (`SAME_INSTRUMENT`/`RELATED_BUT_DISTINCT`/`UNRESOLVED` only, no `NOT_SAME`), never cast to/from `identity_evidence.py`'s separate `FinancialIdentityConclusion`; `IdentityAdjudication` is a frozen dataclass with no update path, migration `0023`'s own table has no UPDATE/DELETE anywhere and a `CHECK` constraint enforcing the three-value vocabulary; every required field (relied/contradictory `EvidenceSignal` ids via a real 1:N `identity_adjudication_signal` table, reviewer, rationale, effective period, `unresolved_qualifications`, `human_reviewer_attested`) is enforced non-empty/True in `__post_init__`; `_validation_failures()` (shared verbatim between `preview_adjudication()` and `record_adjudication()`, so the two can never disagree) blocks stronger dispositions on missing material dimensions, authoritative contradiction, `UNKNOWN` applicability, incompatible known periods, and an absent adjudication `effective_from` — all confirmed by reading the gate logic directly, not the docstring; `UNRESOLVED` is exempt from every gate and never itself blocked; the evidence-page template has no `checked` attribute on any disposition radio (legend states this explicitly: "none selected by default; UNRESOLVED is always available"); `preview_adjudication()` never calls `.save()` anywhere in its body; AC-FID-04 disclosure (`103de84`) independently confirmed to fire on a bare, unmodified GET before any selection, and confirmed scoped per identity dimension by a dedicated cross-dimension-control test, both read in full; `IdentityAdjudicationService`'s own class docstring and every method body confirmed to hold `MatchCandidateRepository` read-only (`.get()` only) and never call any `resolve_*`/`reverse_*` method or `.save()` on a candidate. **Live database independently queried**: `PRAGMA user_version=23` (migration `0023` active), `identity_adjudication`/`identity_adjudication_signal` tables present, both 0 rows (no live adjudication recorded yet — consistent with a just-shipped capability, not a defect). `observation`/`import_run` grew (+21/+27 rows) since the last check, traced and confirmed to be unrelated, purely additive new activity (oldest timestamps unchanged, newest rows are sequential new ids with a real, later timestamp) — not a rewrite, and structurally impossible for this capability to cause (`identity_adjudication_service.py` has zero references to `Observation`/`ImportRun` anywhere). `entity_change_log` unchanged. **UIUX evidence chain read in full, status treated exactly as characterized, independently cross-checked against my own source reading**: `057` (original FAIL — two live-reproduced, root-caused defects: missing CSRF field made the confirm step totally unreachable; `required_dimensions` never wired through from the web layer, silently disabling the missing-dimension/`UNKNOWN`-applicability/incompatible-period gates in production) → `058` (revalidation — both defects PASS/fixed, confirmed via direct database query not UI text; AC-FID-17 keyboard PASS via real UIA focus tracking; AC-FID-18 PASS for every control captured before a genuine NVDA/environment malfunction interrupted the pass, disclosed plainly, not glossed over; AC-FID-19/20 explicitly left attempted-not-confirmed, not rounded to PASS) → `059` (AC-FID-04's remaining gap, root-caused: the domain-layer `diagnose()` function already existed and was correct, simply never called from the web route) → `060` (AC-FID-04 revalidation, PASS, three reproduction cases re-run and confirmed, explicit regression check that the fix doesn't duplicate or replace the pre-existing blocking-reason path). **Resulting gate assessment**: Gate A (technical) — PASS, this review. Gate B (UIUX) — PASS for every application-level defect and for AC-FID-17/18; AC-FID-19/20 and the one radio-announcement sub-check remain genuinely open, attributed to a disclosed NVDA/shared-desktop tooling malfunction, not an application FAIL — Gate B is not fully dischargeable until that follow-up NVDA pass lands. Gates C/D not addressed by this review. **No remaining application-level discrepancy** between what `103de84` ships and what `056`'s (as revised) acceptance criteria require. **Not recorded as a gate-state change in `ACTION_PLAN.md`** — this task requested a report, not a project-state update, consistent with this session's own established distinction between a conformance-review turn and a "record Gate A/C PASS" turn. **Does not claim the unresolved accessibility checks have passed. No HistFinTS or `histfints_uiue` file modified — read-only throughout.** |
| 2026-09-01 | **INC-5's live Yahoo/FRED provider-event capture capability recorded CLOSED/ACCEPTED — Gate D (PO) now PASS, per PO's direct instruction, attributed to its actual owning authority.** This is the final closure event for the bounded capture capability only. Re-verified live one final time before recording, no drift: `histfints` `HEAD` still `5eafe66`, working tree clean except unrelated same-day BYMA output; `provider_event` still exactly **859 rows**. **`ACTION_PLAN.md` §13 updated**: State line now shows the capture capability CLOSED/ACCEPTED (A/C/D PASS-ACCEPT, B deferred to first user-facing surface unchanged); a Gate D section added stating the closure scope precisely as instructed — preserves the 859 real captured events as operational evidence, the evidence-only boundary, the capture-run provenance limitation (shared `acquired_at`, explicitly **not** equivalent to `ImportRun`'s FK pattern, restated at closure exactly as precisely as at Gate A), the original zero-event attempt as a failed execution not evidence, and the delegation-only wiring fix with unchanged retry semantics; explicitly does not broaden into comparability/causal-attribution/adjudication and does not reopen INC-12/13/15 or INC-4's own bounded closure. §5 master row and §8 baseline entry added; §20 updated to match. **Gate A's and Gate C's own records are unedited by this closure.** **No HistFinTS or `histfints_uiue` file modified — read-only throughout.** |
| 2026-09-01 | **INC-5 (Corporate-action and economic-event evidence) recorded Gate A PASS + Gate C PASS (DFA) — Gate D (PO) open, not CLOSED/ACCEPTED.** Per SE directive. Independent conformance review of `histfints@5eafe66` and the live database, verified directly rather than trusted. **Wiring fix confirmed delegation-only**: the diff is exactly one production file, `providers/retry.py` — a single `__getattr__` delegating to the wrapped client; `fetch()` (the only retry-wrapped method) unaffected by construction (`__getattr__` only fires after normal lookup fails), independently confirmed by a dedicated test proving a delegated method's own error is not retried (`sleeps == []`). Zero other production files touched, structurally confirming event semantics/provider selection/Series identity/historical observations are all untouched. **Original failed zero-event attempt confirmed correctly diagnosed as an execution failure, not evidence**: the pre-fix `AttributeError` was caught before either capture service's event-construction/save loop ever ran (read both services in full) — no spurious row was ever created, and no workbench record ever characterized the silent-empty result as genuine evidence. **Live results independently verified via direct SQL**: `provider_event` totals **859 rows exactly** — 4 `SPLIT` + 57 `DIVIDEND` (series 33, Apple Inc.) + 798 `REVISION` (series 1, US Unemployment Rate) — matching the reported breakdown precisely, types and providers explicitly distinguishable at the schema level. **Row-level traceability confirmed field by field** on real sample rows (provider, Series, event date, capture timestamp, provider's own identifier, retained structured representation, provenance note); one nuance named precisely rather than glossed over — no dedicated capture-run entity/FK exists, "captured in run Z" is reconstructible via the shared `acquired_at` timestamp per capture call (confirmed: each call's events share one exact timestamp), a real but implicit traceability mechanism, not identical to `ImportRun`'s explicit pattern. **No historical rows rewritten**: `observation`/`import_run` show identical counts and `max(updated_at)` to every prior check this session, both timestamps predating this work entirely; `entity_change_log` unchanged. **Evidence-only boundary confirmed by reading both capture services in full**: zero references to `Observation`, reconciliation, price repair, causal attribution, comparability, or adjudication anywhere — each service only fetches, de-duplicates, and saves an immutable row. **`provider_event`'s own schema structurally incapable of asserting anything beyond a reported fact** — no adjustment-correctness/comparability/causal-attribution/adjudication field exists anywhere in it, confirmed by reading the full `CREATE TABLE`. **Methodological boundary preserved exactly, per instruction, and confirmed true of the implementation, not merely restated as policy.** **`ACTION_PLAN.md` §13 updated**: State line now shows Gate A/C PASS, Gate D open, not closed; a full Gate A/C record added covering every verified property. **Does not mark INC-5 CLOSED/ACCEPTED. No HistFinTS or `histfints_uiue` file modified — read-only throughout.** |
| 2026-09-01 | **INC-4's `MatchCandidate → EvidenceSignal` capability recorded CLOSED/ACCEPTED — Gate D (PO) now PASS, per PO's direct instruction, attributed to its actual owning authority. INC-4 as a whole remains ACTIVE, not closed.** This is the final closure event for the bounded capability only: all three applicable gates now disposed (A — SDT-WB PASS, `613638a`; C — DFA PASS; D — PO ACCEPT). Re-verified live one final time before recording, no drift: `histfints` `HEAD` still `0a4893d`, working tree clean except unrelated same-day BYMA output; `PRAGMA user_version=22` and `evidence_signal` table present, unchanged; `evidence_signal` still 0 rows. **`ACTION_PLAN.md` §12 updated**: State line now distinguishes INC-4's overall `ACTIVE` status from the bounded capability's own `CLOSED/ACCEPTED` disposition; a Gate D section added stating the closure scope precisely as instructed — closes only the Tier 0/1/2 retention/gathering capability, individual signal preservation, DFA-approved independence semantics, and migration `0022`'s live activation; explicitly does **not** close Tier 3, Resolve/adjudication semantics, INC-4's broader financial-identity-adjudication question, or authorize any automatic identity resolution; explicitly does not reopen INC-12/13/15. **The "no real production signal yet" condition preserved exactly as a standing note, not a reopening trigger**, per explicit instruction — a future naturally-occurring case is additional real-data validation only. §5 master row updated to show the distinction (`ACTIVE` overall, bounded capability `CLOSED`, not a blanket `CLOSED`); §8 gained a terse baseline entry restating the reusable lessons (additive evidence retention beside an unchanged legacy field; retaining non-winning tiers as inspectable, not discarded; a genuinely-empty real-world-occurrence table is not itself a defect when independently traceable; a bounded sub-capability closure inside a larger open increment must state its own boundary explicitly); §20 updated to match. **Evidence chain preserved exactly**: Gate A's and Gate C's own records (`613638a`, the prior DECISIONS.md entry) are unedited by this closure. **No HistFinTS or `histfints_uiue` file modified — read-only throughout.** |
| 2026-09-01 | **INC-4's `EvidenceSignal` (1:N technical-evidence) capability recorded Gate A PASS + Gate C PASS (DFA) — Gate D (PO) open, workstream not marked CLOSED/ACCEPTED.** Per SE directive. Preserved the exact, narrow scope of what passed — the `MatchCandidate → EvidenceSignal` retention mechanism, not INC-4's own broader financial-identity-adjudication question, which remains open and unchanged. **Independently re-verified before recording, not taken on the relayed claims alone**: live `histfints.db` `PRAGMA user_version=22` (was `21` at the prior `613638a` review) — migration `0022` confirmed successfully activated; `evidence_signal` table confirmed to exist with **0 rows**. Traced why, not merely accepted the zero-signal claim: all 9 currently-unresolved `MatchCandidate` rows carry `evidence_tier=3` (Tier-3-only at original discovery — genuinely no Tier 0/1/2 evidence existed for their subjects then, and a re-check finds the same honest nothing now), confirmed live and matching the new `gather_evidence_for_candidate()` capability's own dedicated test for exactly this case (`test_gather_evidence_for_candidate_never_creates_a_new_signal_when_tiers_0_1_2_all_still_miss`, read in full). **New commit reviewed**: `histfints@0a4893d` (`gather_evidence_for_candidate()` — the bounded, one-candidate-at-a-time live-activation path; never creates a new candidate, never touches `evidence_tier`/`rule_reference`, never resolves — confirmed by reading the method and its four dedicated tests in full). Full suite re-confirmed: **1512 passed, 0 failed** (up from 1506 at the prior review). Resolve/adjudication semantics re-confirmed unchanged live — the 4 already-resolved `MatchCandidate` rows are byte-identical to before, `entity_change_log` still contains zero `MatchCandidate`-type entries ever. Tier 3 confirmed not validated or expanded — `0a4893d`'s diff touches only `catalog_discovery_service.py`/`cli.py`/tests. **UIUX `055` independently verified and treated exactly as instructed — confirmation only, not a manufactured gate**: read in full, a bounded readiness assessment finding existing Discover/Resolve UI contracts already sufficient, no new UX spec needed; independently confirmed neither `a113456` nor `0a4893d` touches any `presentation/templates/*.html` or `web.py` file, so no UI validation gate was owed and none was invented. **`ACTION_PLAN.md` §12 updated**: State line now states Gate A PASS / Gate C PASS (DFA, scoped to the `EvidenceSignal` capability) / Gate D open / not CLOSED/ACCEPTED; a full Gate A/C record added preserving each of the nine claimed properties individually, each independently re-checked rather than restated. **Standing note preserved**: no real production Tier 0/1/2 signal has yet occurred; a future naturally-occurring case remains owed as the first genuine real-data validation. **Does not mark INC-4 CLOSED/ACCEPTED. No HistFinTS or `histfints_uiue` file modified — read-only throughout.** |
| 2026-09-01 | **Independent read-only INC-4 implementation conformance review of `histfints@a113456` (EvidenceSignal, MatchCandidate's 1:N evidence child, DFA-approved Tier 0/1/2 methodology) — PASS, no migration applied, no gatherer run against the live database.** Per SE directive. Verified everything directly, not taken on the relayed summary: `HEAD` matches, working tree clean except unrelated same-day BYMA evidence-collection output; full suite **1506 passed, 0 failed**. **Migration/live-DB constraint independently confirmed honored**: live `histfints.db`'s `PRAGMA user_version=21`, one behind migration `0022`'s own number — no `evidence_signal` table exists live, confirmed by direct query. Read `connect()`'s own code (`apply_pending_migrations()` runs unconditionally on every connection, the same automatic mechanism every prior migration in this codebase has used) — flagged as a semantic note, not a defect: migration `0022` is purely additive (`CREATE TABLE`, no `ALTER`, no backfill — confirmed by reading the SQL directly) and will apply itself silently the next time any CLI/web/GUI process connects to the live database (including the 06:00 Daily Import and 17:45 BYMA scheduled tasks), consistent with this project's own established migration convention — not a new risk this increment introduces, but worth naming since it means "not yet applied" is a point-in-time fact, not a standing guarantee. **Twelve required properties, each independently verified**: (1) `MatchCandidate → EvidenceSignal` genuinely 1:N — confirmed at the schema level (`match_candidate_id` indexed, not unique) and structurally separate from `identity_evidence.py`'s own G1/G9 `EvidenceItem`/`EvidenceAuthority` vocabulary (zero import coupling, confirmed by grep; that module's own docstring self-identifies as "the G1/G9 evidence-gated financial-identity capability"); (2) `evidence_tier`/`rule_reference` on `MatchCandidate` structurally untouched by the diff, `CatalogDiscoveryService` only calls `.save()` on the candidate then separately persists signals — never rewritten as canonical; (3) migration `0022`'s own text states "No DEFAULT on any column, no backfill in this migration," confirmed no fabrication path exists anywhere in the diff; (4) `CatalogMatcher.match_identity()` diff read in full — Tier 0/1(from-file)/2 are now all evaluated for every subject regardless of which wins, only Tier 3 stays gated behind "nothing else fired," and the one real cost gate (the rate-limited external `bridge()` call) remains conditioned on "Tier 0 found nothing," exactly as before; (5) `stance` (`SUPPORTS`/`CONTRADICTS`/`INFORMS`) computed per-signal, never collapsed — a new test (`test_tier_0_and_tier_2_agreeing_on_the_same_candidate_are_both_retained`) confirms two agreeing tiers are both retained as distinct signals, not merged; (6) Tier-1 independence (`signals_are_independent()`) implements distinct upstream lineage + same `identity_dimension` + compatible effective period exactly, read in full with its own docstring tracing each clause to its DFA-sourced rule; (7) `DERIVATIVE_PROVIDER`-authority signals excluded from independent pairing outright, confirmed by a dedicated test; (8) `TECHNICAL_PATTERN` (Tier 2) signals excluded from independence "on the same footing," confirmed by a dedicated test with two genuinely different lineage keys still failing independence; (9) Tier 0 candidate-winner logic entirely unchanged — still sufficient alone, no new corroboration requirement added to the winner-selection path; (10) contradictory signals preserved as their own `CONTRADICTS`-stance rows, never scored/suppressed/voted/synthesized — confirmed by two dedicated tests (Tier1-vs-Tier0 winner, Tier2-vs-Tier0 winner); (11) Tier 3 completely unchanged, confirmed to produce zero `EvidenceSignal` rows by a dedicated assertion; (12) zero automatic `resolve_attach`/`resolve_group`/`resolve_merge`/`resolve_set_underlying` calls anywhere in the new/changed code — confirmed by direct grep (only a pre-existing prohibitive comment remains), and the pre-existing "leaves unresolved for Resolve" test still passes unchanged. **All five required demonstration cases independently located and confirmed to prove what they claim**: Tier 0 alone (`test_tier_0_match_resolves_as_attach_when_quantity_matches`); genuine Tier-1 independence, explicitly labeled "Demonstration case 2" in source (`test_distinct_primary_lineage_with_compatible_periods_is_independent`); Tier 2 alone (`test_tier_2_matches_on_normalized_identity_when_no_identifier_tier_matches`); contradictory/historically-unresolved evidence, explicitly labeled "Demonstration case 4" (`test_tier_0_wins_over_tier_1_when_both_would_match`'s new assertions — the winner's own agreeing signal is honestly `INFORMS`, not `SUPPORTS`, since no temporal-validity field exists anywhere in current data, itself independently confirmed by reading `Identifier`/`ProviderSymbol`); a common-upstream/derivative pair correctly not independent, explicitly labeled "Demonstration case 5" (`test_same_upstream_lineage_key_is_never_independent`). **Semantic/migration concerns to report, none blocking**: (a) the automatic-migration-on-next-connect behavior named above — recommend confirming no scheduled task or ad hoc CLI invocation is expected to touch the live database before any additional review is complete, if a delay before live application is wanted; (b) every signal this implementation currently builds has `effective_applicability=UNKNOWN` by construction (no temporal field exists on `Identifier`/`ProviderSymbol` today, confirmed directly) — meaning no two signals produced by today's code can ever satisfy Tier-1 independence's "compatible effective period" clause purely from current data; this is the correct, conservative, missing-evidence-respecting behavior per the DFA rule itself (confirmed in the code's own reasoning), not a defect, but means genuine Tier-1 independence will not appear in real discovery runs until a temporal-evidence source is added — a future increment's scope, not this one's. **Verdict: PASS. Safe to proceed to a bounded live migration/run** — the migration is purely additive and idempotent (matching this project's own established pattern), the code path has full, passing test coverage including all five required demonstration cases, and no live-database or sibling-repository file was modified to produce this review. **Does not claim DFA or PO acceptance.** |
| 2026-09-01 | **INC-6 (provider-level `adjustment_basis` field scope) recorded CLOSED/ACCEPTED — Gate D (PO) now PASS, per PO's direct instruction ("PO has ACCEPTED INC-6"), attributed to its actual owning authority.** This is the final closure event: all three applicable gates now disposed (A — SDT-WB PASS; C — DFA PASS; D — PO ACCEPT). Re-verified live one final time before recording, no drift found: `histfints` `HEAD` still `5619399`, working tree clean except unrelated same-day BYMA output; the finalized inventory unchanged — `FRED=NOT_APPLICABLE`, `Yahoo Finance=SPLIT_ADJUSTED`, `BYMA=NOT_APPLICABLE`, `Finnhub=NULL`, `Twelve Data=UNKNOWN`, `MERVAL=UNKNOWN`, `BYMA EOD=UNKNOWN`. **`ACTION_PLAN.md` updated**: §5 master row `ACTIVE` → `CLOSED`, its own "Governing material" cell explicitly narrowing scope to the provider-level field (not the increment's full original "Required evidence" list — corporate-action context, coverage evidence, etc. remain a broader, separately-gated question, not resolved by this closure); §14 gained the Gate D disposition and a "Closure scope — stated explicitly, not implied" paragraph, matching the pattern used for every prior closure this session; §8 gained a terse INC-6 baseline entry restating the reusable evidence-bar lessons (unevidenced values aren't self-justifying; a reviewed observation-producing path with no basis is `UNKNOWN` not `NULL`; a "0 attributable observations" signal can be a dedup artifact, not a failure signal; a `NULL`-conforming state tied to a live-data condition needs a standing, self-expiring rule, not a one-time classification); §20 updated to match every other closed increment's own entry. **Finnhub's `NULL` boundary condition explicitly preserved as surviving this closure, not discharged by it** — recorded as a standing note binding on whoever next reviews Finnhub's provider row, for as long as its observation history stays empty (currently 23 `FAILED`, 0 `SUCCESS`, all-time). **Evidence chain preserved exactly**: Gate A's and Gate C's own records (including the Twelve Data re-verification and full `entity_change_log` audit trail) are unedited by this closure. **Does not reopen comparability, splicing, corporate-action correctness, or UI scope** — explicitly named as out of this closure's scope, both in the instruction and independently in `ACTION_PLAN.md`'s own closure-scope paragraph. **No HistFinTS or `histfints_uiue` file modified — read-only throughout.** |
