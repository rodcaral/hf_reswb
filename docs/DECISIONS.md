...
| Date | Change |
|---|---|
| 2026-08-18 | Calibration population expansion investigation → **F-032 (H)**: Seven new CEDEAR series (IDs 11323–11329: MU, MSFT, AMD, MELI, QQQ, AMZN, NU) cannot be pooled with the original 5-pair PRIMARY cohort for dispersion analysis. **Root cause: heterogeneous currency/scale bases.** AAPL/BABA/BIDU/UBER/GLD observations range 23k–27k (likely ARS-normalized); new 7 observations range 380–500 (raw USD or different denomination). Pooled CV analysis yields P95 = 1.790 (847% higher than 0.189 baseline), reflecting measurement-unit incompatibility, not statistical dispersion. **File issue `DEFECT-new-cedear-currency-basis.md` to HistFinTS team**, requesting: (a) currency/scale basis verification for 11323–11329; (b) FX normalization if raw USD; (c) population of `adjustment_basis` field (Tranche 2 Item 1) if available. **Recommendation: hold calibration population at original 5-pair PRIMARY cohort (P95 CV 0.189, P90 threshold 0.167) pending schema clarification.** Staleness unchanged (P95 = 3 days for both 5-pair and 12-pair), so that dimension is unaffected. FDA briefing updated with findings and filing status. **Issue blocks expansion; original thresholds remain valid.** |
