...
| Date | Change |
|---|---|
| 2026-08-18 | Calibration population expansion investigation → **F-032 (H)** root cause identified and **Phase 1 (schema correction) complete**. **Root cause: Provider configuration error.** New 7 CEDEAR series (11323–11329) were configured with raw US tickers (MU, MSFT, etc.) instead of `.BA` CEDEAR variants. Yahoo Finance does not support `.BA` CEDEAR exchange variants (returns 422). Result: original 5 pairs correctly fetched as ARS; new 7 incorrectly ingested as raw USD despite schema currency field declaring ARS. **Phase 1 completed:** (a) Currency field corrected to ARS for all 7 series; (b) Provider identifiers confirmed as source of truth (raw tickers); (c) 18,714 old USD observations cleared for clean re-import; (d) Database backed up. **Phase 2 pending:** Historical USD/ARS conversion using BCRA rates (18,714 observations). **Phase 3 pending:** Re-import and validation. **Impact:** Once Phase 2 completes, 12-pair cohort will be homogeneously ARS-denominated and suitable for pooled analysis. Dispersion metrics will return to expected range. **Current status:** Original 5-pair analysis valid; 12-pair expansion blocked until Phase 2 complete; FDA review proceeds on original thresholds (P95 CV 0.189, P90 dispersion 0.167 provisional). |
