...
- **Dated CEDEAR ratios (F-021), adjustment-basis normalisation (F-010), panel eligibility
  (`SPEC_PANEL_ELIGIBILITY.md`).** All real, all separately tracked, none of them this
  increment.
...
| **F-031 — carried-forward bars must be filtered before candidate generation** | **Specified in `SPEC_OBSERVATION_SUITABILITY.md` §5.1 (D-038); not implemented, and the D-035 freeze holds.** Series 11312 steps 10× at 2001-12-12 with *every* bar on both sides zero-volume, OHLC-collapsed and equal to its predecessor. Run on raw rows, this detector generates it as a candidate and — the legacy `correction` table being empty at that date — returns *not explained by captured evidence*, quarantining a span on two rows that represent no trade. Rule for the successor increment: candidate generation runs on the suitability-filtered series; a boundary whose endpoints are both `NO_TRADE_REPORTED` is **not** a candidate; a boundary with one such endpoint is a candidate whose `discontinuity_calculation` must record the endpoint's classification. Affects §4.2 candidate generation and the meaning of §4.3's verdict, not the verdict vocabulary. |
