...
**This citation was written against `ACTOR_AND_MODEL_INTERACTION_RULES.md@c14b7b7`. If you have
...
| `docs/HISTFINTS-BRIEF-v2.md` | Corrected technical brief: schema, known defects, adjustment basis, V0 data reality. Supersedes the original consuming-team brief, which was twice misread as exhaustive during review and caused a retracted finding. **Currency caveat, 2026-09-04:** the brief's own header says "current as of 2026-08-15," and its last real content commit was 2026-08-26 (a reference fix only) — it has not kept pace with `DECISIONS.md`, `DATABASE_SCHEMA.md`, or the increment closures since. Read it for orientation, not as the current-state authority its own "Start here" framing implied; `DECISIONS.md` and `DATABASE_SCHEMA.md` are the current sources. |
